...
NOISE = [
    ...
    "a1b2c3d4e5f6:connect device: 192.0.2.10",
    "a1b2c3d4e5f6:connect success: 192.0.2.10",
...
def test_a_record_that_cannot_be_formatted_is_emitted(filt):
    ...
    bad = logging.LogRecord(
        LOGGER_NAME, logging.WARNING, __file__, 1, "%s %s", ("only-one-arg",), None
    )
...
def test_discover_chatter_is_dropped(filt):
    ...
    for msg in (
        ...
        rec = logging.LogRecord(
            "aidot.discover", logging.WARNING, __file__, 1, msg, None, None
        )
