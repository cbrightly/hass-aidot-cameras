...
async def test_a_webrtc_view_waits_past_the_hls_budget_for_the_publisher():
    ...
    client = _client(
        [_streams([PLACEHOLDER])] * 41 + [_streams([PLACEHOLDER, PUBLISHER])]
    )
...
async def test_the_dtls_serve_wait_is_sized_by_the_caller_too():
    ...
    with (
        ...
        patch.object(
            ...
            return_value="http://127.0.0.1:18931/x.ts",
        ),
    ):
        await cam.stream_source()
        view_timeout = (
            cam.coordinator.device_client.async_wait_serve_ready.await_args.kwargs[
                "timeout"
            ]
        )
