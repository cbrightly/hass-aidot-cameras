...
def _coord_no_lan(**kw):
    """Coordinator whose device never reported over the LAN."""
    c = _coord(**kw)
    c.has_lan_state = False
    return c


def test_no_lan_state_reports_unknown_not_off():
    # A bulb that is powered on but off-LAN must not be published as "off":
    # the library status holds only defaults, and control is LAN-only.
    light = AidotLight(_coord_no_lan(active_color_mode=None, cct=3000))
...
def test_lan_state_reports_real_values():
    coord = _coord(active_color_mode="cct", cct=3000)
    coord.has_lan_state = True
    light = AidotLight(coord)
    assert light.is_on is True
    assert light.color_temp_kelvin == 3000


async def test_command_without_lan_raises_clear_error():
    # Upstream raises a bare ConnectionError when there is no LAN session; the
    # entity must translate it instead of letting it surface as a 500.
    from unittest.mock import AsyncMock, MagicMock
    ...
    try:
        ...
        assert "not reachable over the local network" in str(exc)
    else:
