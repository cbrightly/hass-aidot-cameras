...
async def test_stream_source_sdes_push_soft_fails_without_go2rtc():
    # In push mode a down go2rtc is a hard dependency: soft-fail (None) so HA
    # shows the still image and retries, instead of handing out a dead URL.
    cam = _make_camera(is_sdes=True, stream_rtsp_url=None)
    cam._setup_complete = True
...
async def test_stream_source_sdes_pull_legacy_when_push_disabled():
    # sdes_push off: the legacy HTTP pull serve behaviour is unchanged.
    cam = _make_camera(is_sdes=True, stream_rtsp_url=None,
...
# _startup_prewarm
# --------------------------------------------------------------------------- #
async def test_startup_prewarm_is_off_by_default():
    # Startup pre-warm opens a session on EVERY mains camera at every restart and
    # reload, with no viewer present - which is exactly what "why are all my
    # cameras streaming?" looks like, and each session holds a concurrency slot
    # and keeps the camera decrypting. It is opt-in now.
...
async def test_startup_prewarm_warms_mains_camera_when_enabled():
    # With the option on, a mains camera still warms in the background
    # (delegating to the idempotent _prewarm_stream).
