...
async def test_stored_sdes_push_off_is_ignored_not_honoured():
    ...
    cam = _make_camera(is_sdes=True, stream_rtsp_url=None, options={"sdes_push": False})
    cam._setup_complete = True
    cam._publish_to_go2rtc = AsyncMock(return_value="rtsp://go2rtc/x")
    cam._await_serve_listening = AsyncMock(return_value=True)
...
async def test_stream_source_cancelled_propagates():
    ...
    with (
        patch.object(camera_mod, "_GO2RTC_ENABLED", False),
...
async def test_the_listing_does_not_hold_up_the_stream():
    ...
    tasks = []
    cam.hass.async_create_task = lambda coro: tasks.append(asyncio.ensure_future(coro))
    await cam._prewarm_stream()
...
async def test_push_prewarm_stops_when_go2rtc_will_not_take_the_stream():
    # _publish_to_go2rtc returns None rather than raising when go2rtc is disabled
    # or the PUT fails (restarting, or reachable only over a unix socket). Pushing
    # anyway aims RTSP at a name go2rtc does not know: the publish is refused and
    # the ffmpeg plus the keepalive retry loop churn against a camera nobody is
    # watching. The existing prewarm tests mock this as always succeeding, so the
    # failure path had no coverage at all.
    cam = _make_camera(is_sdes=True, stream_rtsp_url=None)
    cam._publish_to_go2rtc = AsyncMock(return_value=None)
