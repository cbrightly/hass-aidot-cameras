...
async def test_live_publisher_is_not_re_registered():
    ...
    assert url == "rtsp://127.0.0.1:8554/aidot_aaaa3333bbbb"
...
@pytest.mark.asyncio
async def test_unreachable_go2rtc_still_registers_and_soft_fails():
    ...
    but still attempts the registration, whose failure is what soft-fails the
    view to None so HA shows the still image and retries."""
    cam = _make_camera(is_sdes=True, stream_rtsp_url="rtsp://127.0.0.1:8554/live")
    streams_empty: dict = {}
    client = MagicMock()
