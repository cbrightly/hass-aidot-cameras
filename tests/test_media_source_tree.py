...
class _DC:
    def __init__(self, name="Front Door Cam"):
        from types import SimpleNamespace
...
async def test_a_day_folder_lists_every_event_not_the_first_ten(hass):
    ...
    with p:
        ...
        node = await src.async_browse_media(_Item(day_id))
...
async def test_a_camera_offers_two_sources(hass):
    ...
    assert [c.title for c in node.children] == ["Cloud", "On device"]
...
def _sd_day() -> str:
    ...
    return dt_util.as_local(datetime(2026, 8, 11, 12, 0, tzinfo=UTC)).strftime(
        "%Y-%m-%d"
    )
