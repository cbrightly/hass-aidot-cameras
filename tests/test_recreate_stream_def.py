...
def _cam(dev_id="dev-1", push=False):
    c = MagicMock()
    c.coordinator.device_client.device_id = dev_id
    c._go2rtc_name = "aidot_dev1"
    c._rtsp_name = "aidot_dev1"
    c.hass = MagicMock()
    # These tests are about the remove-before-PUT ordering, not push mode, but
    # _publish_to_go2rtc asks which mode it is in to decide whether the source
    # is the serve URL or the inert placeholder - so the double has to answer.
    c._push_mode = AsyncMock(return_value=push)
    return c
...
async def test_removes_definition_only_for_flagged_cameras(unstable, expect_remove):
    ...
    with (
        ...
        out = await cam_mod.AidotCamera._publish_to_go2rtc(
...
async def test_remove_happens_before_the_put():
    ...
    client.ensure_stream = AsyncMock(
        side_effect=lambda *a, **k: calls.append("put") or True
    )
    client.remove_stream = AsyncMock(
        side_effect=lambda *a, **k: calls.append("delete") or True
    )
    client.rtsp_url = MagicMock(return_value="rtsp://go2rtc/aidot_dev1")
    with (
        patch.object(cam_mod, "_GO2RTC_ENABLED", True),
