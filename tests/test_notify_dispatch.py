...
def test_camera_targets_override_global():
    opts = _options(
        targets=["global"], cameras={DEV: {"events": "all", "targets": ["mine"]}}
    )
...
def test_empty_camera_targets_fall_back_to_global():
    opts = _options(
        targets=["global"], cameras={DEV: {"events": "person", "targets": []}}
    )
    cfg = resolve_camera_config(opts, DEV)
...
def test_configured_cooldown_title_message():
    opts = _options(
        cooldown_s=5, title="T {camera}", message="M", cameras={DEV: {"events": "all"}}
    )
    cfg = resolve_camera_config(opts, DEV)
...
def test_render_substitutes_known_placeholders():
    assert (
        render("{camera} {event_title}", {"camera": "Porch", "event_title": "Person"})
        == "Porch Person"
    )
...
def test_payload_time_uses_event_time_when_present():
    ...
    expected = dt_util.as_local(
        dt_util.utc_from_timestamp(event_time_ms / 1000.0)
    ).strftime("%H:%M")
...
@pytest.mark.parametrize(
    "bad_event", [{"eventCode": "4"}, {"eventCode": "4", "eventTime": "abc"}]
)
...
def _entry(hass, cams, **cfg):
    ...
    entry.async_create_task = lambda _hass, coro, name=None: hass.async_create_task(
        coro
    )
    return entry
...
def _capture_notify(hass, name):
    ...
    hass.services.async_register(
        "notify", name, lambda call: calls.append(dict(call.data))
    )
    return calls
...
async def test_event_sends_to_camera_targets(hass):
    ...
    entry = _entry(
        ...
        {DEV: cam},
        targets=["global"],
        cameras={DEV: {"events": "all", "targets": ["phone"]}},
    )
...
async def test_person_only_camera_ignores_plain_motion(hass):
    cam = _FakeCamCoord(DEV)
    entry = _entry(
        hass, {DEV: cam}, targets=["phone"], cameras={DEV: {"events": "person"}}
    )
    phone = _capture_notify(hass, "phone")
    AidotMotionNotifier(hass, entry).start()
...
async def test_camera_set_to_off_sends_nothing(hass):
    cam = _FakeCamCoord(DEV)
    entry = _entry(
        hass, {DEV: cam}, targets=["phone"], cameras={DEV: {"events": "off"}}
    )
    phone = _capture_notify(hass, "phone")
    AidotMotionNotifier(hass, entry).start()
    cam.fire({"eventCode": "4"})
...
async def test_no_target_warning_rearms_after_a_successful_send(hass, caplog):
    ...
    entry.options = _options(
        targets=["phone"], cooldown_s=0, cameras={DEV: {"events": "all"}}
    )
    cam.fire({"eventCode": "1"})
    await hass.async_block_till_done()
    assert len(phone) == 2
...
async def test_unknown_target_does_not_block_the_others(hass, caplog):
    cam = _FakeCamCoord(DEV)
    entry = _entry(
        hass, {DEV: cam}, targets=["missing", "phone"], cameras={DEV: {"events": "all"}}
    )
    phone = _capture_notify(hass, "phone")
    AidotMotionNotifier(hass, entry).start()
    cam.fire({"eventCode": "1"})
    ...
    assert len(phone) == 1
    assert any(
        "missing" in r.getMessage() and r.levelname == "WARNING" for r in caplog.records
    )
...
async def test_scheduling_failure_is_logged_and_leaves_no_pending_coroutine(
    hass, caplog
):
    ...
    cam = _FakeCamCoord(DEV)
    entry = _entry(
        hass, {DEV: cam}, targets=["phone"], cameras={DEV: {"events": "all"}}
    )
