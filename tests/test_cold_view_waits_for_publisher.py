...
background afterwards. The gain is that a battery camera can serve HLS at all.

The cost this used to name - "one bad first click on a genuinely cold WebRTC
open" - **is no longer the trade**, and the file it describes has moved on:
`_publisher_wait_budget` now applies this short cap to the HLS path ONLY, while
the view path a card tap uses gets `_VIEW_PUBLISHER_WAIT_BATTERY_S` (75 s) and
has no Home Assistant timeout above it at all. Measured 2026-09-12 on two
charged A001513s, the view path answered first try in 0.3 s and 6.2 s.

What is still true is the HLS limit: a cold battery camera cannot beat the 10 s
clock, so `camera.record` and HLS on a cold battery camera do still fail and
retry. Testing "does the first click work" with `camera.record` measures that
cap rather than the card, which is exactly how this comment misled a reader.
...
async def test_pull_mode_keeps_the_serve_port_probe():
    ...
    with (
        patch.object(cam_mod, "_GO2RTC_ENABLED", False),
        p[0],
        p[1],
        p[2],
        p[3],
