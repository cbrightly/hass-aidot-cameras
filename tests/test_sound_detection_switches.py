...
class _Client:
    def __init__(self, sound=None, battery=False, has_methods=True, set_result=True):
        self.device_id = "dev1"
...
class TestTheFillCadence:
    ...
    def test_a_complete_set_settles_to_the_slow_cadence(self):
        ...
        from custom_components.aidot.coordinator import (
            ...
            EXTRAS_REFRESH_SECONDS,
        )

        # Built from the coordinator's own key tuple, not a hard-coded list:
        # this used to name the three keys of the day and started failing the
        # moment a fourth was added, which says nothing about the cadence.
        complete = {k: {} for k in AidotCameraUpdateCoordinator._EXTRAS_KEYS}
        c = self._coord_with_hass(complete)
        c._schedule_camera_extras()
...
class TestTheInFlightSlot:
    ...
    def test_a_fetch_that_never_suspends_leaves_the_slot_free(self):
        async def go():
            ...
            assert c._extras_inflight is False, (
                "slot still claimed after a finished fetch"
            )
    ...
    def test_a_fetch_that_suspends_still_holds_the_slot(self):
        ...
        async def go():
            ...
            assert c._extras_inflight is False, (
                "slot not released when the fetch finished"
            )
