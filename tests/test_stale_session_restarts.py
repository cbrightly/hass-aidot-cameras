...
async def _stream_source(cam: AidotCamera, producers: list | None):
    ...
    with (
        ...
        # These tests hold go2rtc at "no publisher", so the cold-open wait would
        # run its full real-world budget. Shrink it - what is under test here is
        # the restart decision, not the wait.
