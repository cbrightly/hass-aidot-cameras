...
Both were verified on hardware by read-back before being given an entity. The third, `lightBehavior` (Constant/Flash), was held back until
2026-09-08 because the write acked and the value never changed. That turned out
to be a battery camera never getting time to wake before the command arrived
(library `1.0.0rc18`), not anything about the attribute, and it now lands on a
sleeping camera. It ships here with the same profile gate as the duration.
"""
...
class _Client:
    def __init__(self, declared=("LingerDuration", "Dimming", "lightBehavior")):
        self.device_id = "dev1"
    ...
    async def async_set_light_behavior(self, behavior):
        self.calls.append(("behavior", behavior))
        return True


def _coord(status, declared=("LingerDuration", "Dimming", "lightBehavior")):
...
def test_the_duration_select_reports_what_the_camera_said():
    ...
    e = _entity(
        AidotCameraSelect, coord, _desc(CAMERA_SELECTS, "light_linger_duration")
    )
...
def test_the_behavior_select_offers_exactly_what_the_camera_declares():
    """Constant and Flash, the two values in the model profile - no more."""
    d = _desc(CAMERA_SELECTS, "light_behavior")
    assert list(d.options) == ["constant", "flash"]


def test_the_behavior_select_reports_what_the_camera_said():
    coord = _coord(_Status(light_behavior="flash"))
    e = _entity(AidotCameraSelect, coord, _desc(CAMERA_SELECTS, "light_behavior"))
    assert e.current_option == "flash"


def test_a_behavior_the_camera_reports_but_we_do_not_offer_reads_unknown():
    """The library carries an unmodelled value through as itself; do not show it."""
    coord = _coord(_Status(light_behavior="7"))
    e = _entity(AidotCameraSelect, coord, _desc(CAMERA_SELECTS, "light_behavior"))
    assert e.current_option is None


def test_choosing_a_behavior_sends_the_name_the_library_expects():
    coord = _coord(_Status(light_behavior="constant"))
    desc = _desc(CAMERA_SELECTS, "light_behavior")
    import asyncio

    asyncio.run(desc.async_select_option_fn(coord.device_client, "flash"))
    assert coord.device_client.calls == [("behavior", "flash")]


def test_the_behavior_select_needs_the_MODEL_to_declare_it():
    """Same gate as the duration, and for the same reason.

    An A000088 carries a `lightBehavior` value in its cloud properties while the
    camera, asked over LAN, has no such attribute at all. Only the L2 line
    declares it.
    """
    d = _desc(CAMERA_SELECTS, "light_behavior")
    assert (
        d.is_supported(
            _Status(light_behavior="constant"), _Client(declared=("Dimming",))
        )
        is False
    )
    assert (
        d.is_supported(
            _Status(light_behavior="constant"), _Client(declared=("lightBehavior",))
        )
        is True
    )
...
def test_every_new_control_is_gated():
    ...
    for descs, key in (
        (CAMERA_SELECTS, "light_linger_duration"),
        ...
        assert d.is_supported(_Status(), _Client()) is False
    assert (
        _desc(CAMERA_SELECTS, "light_linger_duration").is_supported(
            _Status(light_linger_duration=30), _Client()
        )
        is True
    )
    assert (
        _desc(CAMERA_NUMBERS, "light_brightness").is_supported(
            _Status(light_brightness=100), _Client()
        )
        is True
    )
...
def test_the_duration_select_needs_the_MODEL_to_declare_it_not_the_cloud():
    ...
    assert (
        _desc(CAMERA_SELECTS, "light_linger_duration").is_supported(
            _Status(light_linger_duration=30), reports_it_but_does_not_declare_it
        )
        is False
    )
...
def test_brightness_is_NOT_profile_gated_and_that_is_deliberate():
    ...
    assert (
        _desc(CAMERA_NUMBERS, "light_brightness").is_supported(
            _Status(light_brightness=80), declares_neither
        )
        is True
    )
...
def test_an_unreadable_profile_does_not_silently_remove_the_control():
    ...
    assert (
        _desc(CAMERA_SELECTS, "light_linger_duration").is_supported(
            _Status(light_linger_duration=30), _NoInfo()
        )
        is True
    )
...
async def test_the_23_0_duration_select_is_removed_where_it_could_never_work(hass):
    ...
    doomed = reg.async_get_or_create("select", DOMAIN, "camA_light_linger_duration")
    keeper = reg.async_get_or_create("select", DOMAIN, "camB_light_linger_duration")
