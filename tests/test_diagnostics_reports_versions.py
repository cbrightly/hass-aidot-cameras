...
class _Hass:
    """Records what diagnostics hands to the executor."""
...
async def test_the_diagnostics_download_carries_the_versions():
    ...
    diag = await async_get_config_entry_diagnostics(_Hass(), _Entry())
    assert "versions" in diag, diag.keys()
    assert diag["versions"]["python-aidot-cameras"] is not None


async def test_the_version_lookup_runs_off_the_event_loop():
    """importlib.metadata.version() lists site-packages and reads METADATA.

    Called on the loop, Home Assistant logs "Detected blocking call to listdir
    ... inside the event loop" three times per download (seen on HA 2026.9.2,
    diagnostics.py line 41). The lookup belongs in the executor.
    """
    from custom_components.aidot.diagnostics import (
        _package_versions,
        async_get_config_entry_diagnostics,
    ...
    hass = _Hass()
    await async_get_config_entry_diagnostics(hass, _Entry())
    assert _package_versions in hass.executor_jobs, hass.executor_jobs
