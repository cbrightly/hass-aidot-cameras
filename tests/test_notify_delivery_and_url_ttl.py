...
def _notifier(service_side_effect=None):
    ...
    n._gate = SimpleNamespace(
        record=lambda *_a, **_k: None, allows=lambda *_a, **_k: True
    )
    return n, hass
...
def test_dispatcher_source_uses_blocking_send():
    ...
    code = [
        ln
        for ln in pathlib.Path(notify_dispatch.__file__).read_text().splitlines()
...
def test_overnight_push_url_still_verifies_nine_hours_later():
    ...
    assert not _verify_sig(
        "dev", "evt", q_old["exp"], q_old["sig"], now=nine_hours_later
    )
...
def test_every_cached_clip_response_carries_the_header():
    ...
    code = [
        ln
        for ln in pathlib.Path(proxy.__file__).read_text().splitlines()
        if not ln.lstrip().startswith("#")
    ]
