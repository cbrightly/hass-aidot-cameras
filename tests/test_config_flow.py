...
def _patch_client(return_value=MOCK_LOGIN_INFO, side_effect=None):
    mock = AsyncMock()
    mock.async_post_login = AsyncMock(
...
async def test_create_entry_on_success(hass: HomeAssistant, mock_setup_entry) -> None:
...
async def test_options_flow_opens_a_menu(hass: HomeAssistant, mock_setup_entry) -> None:
    entry = MockConfigEntry(
        domain=DOMAIN, data={"id": "u"}, options={"serve_port_base": 18600}
    )
...
async def test_streaming_step_saves_and_keeps_other_options(
    hass: HomeAssistant, mock_setup_entry
) -> None:
    entry = MockConfigEntry(
        domain=DOMAIN,
    ...
    result = await hass.config_entries.options.async_configure(
        result["flow_id"], {"next_step_id": "streaming"}
    )
    assert result["type"] == FlowResultType.FORM
...
async def test_notifications_step_saves_global_settings(
    hass: HomeAssistant, mock_setup_entry
) -> None:
...
async def test_notifications_step_keeps_per_camera_settings(
    hass: HomeAssistant, mock_setup_entry
) -> None:
    entry = MockConfigEntry(
        domain=DOMAIN,
        data={"id": "u"},
        options={
            CONF_NOTIFICATIONS: {"cameras": {"dev1": {"events": "all", "targets": []}}}
        },
    )
    entry.add_to_hass(hass)
    result = await hass.config_entries.options.async_init(entry.entry_id)
    ...
    result = await hass.config_entries.options.async_configure(
        result["flow_id"],
        user_input={"targets": ["p"], "cooldown_s": 0, "title": "t", "message": "m"},
    )
    assert result["type"] == FlowResultType.CREATE_ENTRY
    assert entry.options[CONF_NOTIFICATIONS]["cameras"] == {
        "dev1": {"events": "all", "targets": []}
    }
    assert entry.options[CONF_NOTIFICATIONS]["targets"] == ["p"]


async def test_notifications_step_lists_notify_services_but_not_meta_ones(
    hass: HomeAssistant, mock_setup_entry
) -> None:
    hass.services.async_register("notify", "mobile_app_phone", lambda call: None)
    ...
    entry = MockConfigEntry(
        domain=DOMAIN,
        data={"id": "u"},
        options={CONF_NOTIFICATIONS: {"targets": ["stored_one"]}},
    )
    entry.add_to_hass(hass)
    result = await hass.config_entries.options.async_init(entry.entry_id)
    result = await hass.config_entries.options.async_configure(
    ...
    assert "notify" in options  # legacy group target is a legitimate choice
...
def _loaded_cameras(entry, cams: dict[str, str]) -> None:
    ...
    coords = {
        dev_id: SimpleNamespace(
            device_client=SimpleNamespace(info=SimpleNamespace(name=name))
        )
...
async def test_camera_step_aborts_when_nothing_is_loaded(
    hass: HomeAssistant, mock_setup_entry
) -> None:
    ...
    result = await hass.config_entries.options.async_init(entry.entry_id)
    result = await hass.config_entries.options.async_configure(
        result["flow_id"], {"next_step_id": "camera_notifications"}
    )
    assert result["type"] == FlowResultType.ABORT
    assert result["reason"] == "no_cameras"


async def test_camera_step_lists_cameras_by_name(
    hass: HomeAssistant, mock_setup_entry
) -> None:
    entry = MockConfigEntry(domain=DOMAIN, data={"id": "u"}, options={})
    entry.add_to_hass(hass)
...
async def test_camera_notify_step_saves_that_camera_only(
    hass: HomeAssistant, mock_setup_entry
) -> None:
    entry = MockConfigEntry(
        domain=DOMAIN,
        ...
        options={
            CONF_NOTIFICATIONS: {
                "targets": ["g"],
                "cameras": {"dev1": {"events": "all", "targets": []}},
            }
        },
    )
    entry.add_to_hass(hass)
    ...
    result = await hass.config_entries.options.async_configure(
        result["flow_id"], user_input={"camera": "dev2"}
    )
    assert result["type"] == FlowResultType.FORM
...
async def test_camera_notify_step_defaults_to_off_and_shows_stored_values(
    hass: HomeAssistant, mock_setup_entry
) -> None:
    entry = MockConfigEntry(
        domain=DOMAIN,
        ...
        options={
            CONF_NOTIFICATIONS: {
                "cameras": {"dev1": {"events": "person", "targets": ["p"]}}
            }
        },
    )
    entry.add_to_hass(hass)
    ...
    result = await hass.config_entries.options.async_configure(
        result["flow_id"], user_input={"camera": "dev1"}
    )
    defaults = {str(k): k.default() for k in result["data_schema"].schema}
    assert defaults == {"events": "person", "targets": ["p"]}


async def test_camera_notify_without_a_selection_goes_back_to_the_picker(
    hass: HomeAssistant, mock_setup_entry
) -> None:
    from custom_components.aidot.config_flow import AidotOptionsFlow

    entry = MockConfigEntry(domain=DOMAIN, data={"id": "u"}, options={})
    entry.add_to_hass(hass)
    _loaded_cameras(entry, {"dev1": "Garage"})

    flow = AidotOptionsFlow()
    flow.hass = hass
    flow.handler = entry.entry_id
    result = await flow.async_step_camera_notify()
    assert result["type"] == FlowResultType.FORM
    assert result["step_id"] == "camera_notifications"
