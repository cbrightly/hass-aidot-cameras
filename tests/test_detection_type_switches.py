...
_LIVE = {"humanDetect": True, "vehicleDetect": False, "packageDetect": False,
         "petDetect": False, "publicZone": True}
...
def test_publicZone_is_not_a_detection_type():
    ...
    assert set(DETECTION_TYPE_TRANSLATION_KEYS) == {
        "humanDetect", "vehicleDetect", "packageDetect", "petDetect"}
...
def test_an_unanswered_camera_is_unknown_even_with_other_extras_present():
    ...
    sw = _switch(_coord(_Client(None), {"sound": {"glass_Break": True}}),
                 "vehicleDetect")
    assert sw.is_on is None
