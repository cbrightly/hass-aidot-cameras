...
async def test_on_and_off_call_the_right_library_setter():
    ...
    class _C:
        async def async_set_osd_timestamp(self, v):
            calls.append(("osd", v))
            return True

        async def async_set_auto_light(self, v):
            calls.append(("light", v))
            return True

        async def async_set_voice_prompts(self, v):
            calls.append(("voice", v))
            return True

        async def async_set_hdr(self, v):
            calls.append(("hdr", v))
            return True
