...
def test_the_addresses_are_gone():
    ...
    flat = repr(out)
    assert "192.0.2.109" not in flat
...
def test_redaction_does_not_mutate_the_live_session_stats():
    ...
    assert original["nominated"]["local"] == "192.0.2.109:54321", (
...
def test_the_ice_candidate_list_is_redacted_too():
    ...
    health = {
        ...
        "ice": [
            {
                "local": "192.0.2.9:36206",
                "remote": "192.0.2.120:50097",
                "local_type": "host",
                "remote_type": "host",
            },
            {"local": "192.0.2.9:55776", "remote": "10.0.0.4:58724"},
        ],
        "transport": "dtls",
    }
    out = _redact_stream_health(health)
    blob = repr(out)
    assert "192.0.2.9" not in blob and "192.0.2.120" not in blob, (
    ...
    assert health["ice"][0]["local"] == "192.0.2.9:36206"
