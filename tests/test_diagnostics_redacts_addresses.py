...
def test_the_addresses_are_gone():
    ...
    flat = repr(out)
    assert "192.168.7.109" not in flat
...
def test_redaction_does_not_mutate_the_live_session_stats():
    ...
    assert original["nominated"]["local"] == "192.168.7.109:54321", (
...
def test_the_ice_candidate_list_is_redacted_too():
    ...
    health = {
        "nominated": {"local": "192.168.7.9:36206", "remote": "3.230.182.123:50097",
                      "local_type": "host", "remote_type": "relay"},
        "ice": [
            {"local": "192.168.7.9:36206", "remote": "192.168.7.120:50097",
             "local_type": "host", "remote_type": "host"},
            {"local": "192.168.7.9:55776", "remote": "10.0.0.4:58724"},
        ],
        "transport": "dtls",
    }
    out = _redact_stream_health(health)
    blob = repr(out)
    assert "192.168.7.9" not in blob and "192.168.7.120" not in blob, (
    ...
    assert health["ice"][0]["local"] == "192.168.7.9:36206"
