...
### Removed
- **The camera resolution (HD/SD) select.** The cameras ignore it. The command
  is delivered - the library sends `SETSTREAMCTRL` over the live session and
  re-sends it when a session starts - and the encode never changes. Measured by
  reading `videoWidth` off live WebRTC tracks, which is the encode itself rather
  than a scaled snapshot: the A001064 PTZ stayed at 1280x720 under `sd` both
  mid-session and at session start, and an A000088 M3 Pro stayed at 1280x720
  thirty seconds after the change. Two models across both transports.

  The entity accepted a value, restored it across restarts and reported a
  setting the camera had never applied. A control that lies persistently is
  worse than no control, so it is gone. `async_set_resolution` remains in the
  library - the command is correct, and a future firmware may honour it.
