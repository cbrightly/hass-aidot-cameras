...
- **The direct-publish option no longer says the HLS dialog has sound.** Its
  description in the integration's options still promised that Home
  Assistant's HLS dialog "still carries audio" because go2rtc transcodes to
  AAC for it. 2.30.4 stopped offering that transcode - it was what made live
  views play about eleven times too slow - so the dialog is now silent, and
  the option text says so: live WebRTC views carry the camera's own audio, and
  the HLS dialog opened by the Picture cards does not.

- **The README says plainly which views have sound.** A new "Audio" section
  explains that WebRTC cards carry audio and the Picture cards' HLS dialog
  does not, and why; the quick start now gives sound as a second reason to use
  a WebRTC card; and Troubleshooting has an entry for live view playing in
  slow motion, pointing at 2.30.4. The same wording is in the wiki.

No behaviour change: the integration's code is identical to 2.30.4, and it
still requires python-aidot-cameras 1.0.0rc29.

## [2.30.4]

### Fixed

- **Live views no longer play about eleven times too slow.** This integration
  added an `ffmpeg:<stream>#audio=aac` source to every go2rtc stream so Home
  Assistant's HLS player had audio it can carry (its stream component keeps
  aac/mp3 and drops G.711). The go2rtc build the cameras are served by stamps
  that transcode on a 90 kHz clock while advertising `MPEG4-GENERIC/8000`, so
  the audio arrived stretched and every HLS viewer played in slow motion. The
  audio was never lost, only mis-timed. Measured on one camera, same stream,
  same 15 s wall-clock window: the passthrough arrived as a 15.000 s container
  and the transcode as 163.143 s. It was not camera-specific - five of five
  cameras across three models sat between 10.76x and 11.12x. Serving viewers
  through a newer go2rtc did not help either: offered this source, Home
  Assistant's own bundled go2rtc prefers it, because go2rtc serves a consumer
  from the first source whose codecs match, and relays the bad timestamps
  unchanged. The source is no longer offered.

  WebRTC viewers are unaffected and keep audio as G.711 passthrough. Home
  Assistant's HLS dialog now plays at the correct speed without audio, which is
  the deliberate trade - a silent stream at 1x beats an 11x one with sound.

  A camera that publishes rather than being pulled keeps its previous stream
  definition until its next live view, because those are not registered during
  setup; opening the camera once clears it.

## [2.30.3]

### Fixed

- **A camera no longer drops to HLS when go2rtc rejects the register call but
  is still holding the stream.** `_publish_to_go2rtc()` treated any failed
  `ensure_stream()` as "go2rtc cannot serve this" and returned None, which
  sends `stream_source()` down its local-serve fallback - Home Assistant's HLS
  pipeline - instead of go2rtc's low-latency path. go2rtc can reject the PUT
  while the stream stays registered: a duplicate stream key in its own
  `go2rtc.yaml` makes it answer *every* `PUT /api/streams` with
  `400 yaml: unmarshal errors: mapping key "<name>" already defined`, so one
  bad key takes every camera off the low-latency path, not just the camera that
  owns it. Nothing is logged as an error, so the only symptom is a camera that
  appears to play slower than real time. The stream list now settles it, and
  the fallback is reserved for a stream that is genuinely absent.

### Changed

- **Requires `python-aidot-cameras` 1.0.0rc29.** That release carries the
  matching fix for `prefer_go2rtc()`, so both call sites that could drop a
  camera onto the HLS path on a rejected register call are covered.

## [2.30.2]

### Changed

- **Requires `python-aidot-cameras` 1.0.0rc28.** That release stops a transient
  cloud-poll failure logging as an error. The poll re-reads a 600 s window and
  dedupes by event id, so a blip costs nothing; a run that outlasts the window
  still raises an error, and an unexpected failure still does so immediately.
  Motion notifications are unaffected either way - this is about which log
  lines are worth your attention.

## [2.30.1]

### Added

- **Says how to put a password on the go2rtc stream, and what the URL then
  looks like.** 2.30.0 noted that go2rtc could be made to authenticate and left
  it at that. It can, the URL is the ordinary
  `rtsp://user:pass@host:8554/aidot_<id12>`, and the README and wiki now show
  the config for it - verified against the go2rtc version Home Assistant ships:
  from another machine, missing or wrong credentials are refused and the right
  ones play. Loopback is exempt from that check, so a test run on the go2rtc
  host itself succeeds either way and proves nothing; that is called out too.
  Also states plainly that the cameras have no RTSP server of their own, since
  `rtsp://user:pass@camera` is the shape people reasonably expect.

## [2.30.0]

### Added

- **Documented what registering a camera with go2rtc exposes.** To serve the
  WebRTC live view, each camera becomes a go2rtc stream named `aidot_<id12>`,
  and anything that can reach go2rtc can then pull that camera as plain RTSP
  with no Home Assistant login. That is useful - point Frigate, an NVR or VLC
  at it and it shares the one session the integration already holds, rather
  than taking the camera's single viewer slot - and it is also the whole
  exposure, so both halves are now written down in the README and in a new
  wiki page. No behaviour changed: this is how the live view has always
  worked, it had just never been stated.

## [2.29.3]

### Fixed

- **The direct-publish option no longer claims HLS loses audio.** It did when
  the option first shipped; 2.26.0 fixed it by giving go2rtc an on-demand AAC
  source for that player, and the text was never updated. Verified on all three
  camera models through Home Assistant's own HLS path.

## [2.29.2]

### Fixed

- **A published camera's go2rtc stream no longer names the local port it never
  binds.** go2rtc will not hold a stream with no source, so the definition has
  to exist before the publish - and it was created with the camera's own local
  serve URL, which is exactly the port push mode does not listen on. That left
  a definition pointing at an address nothing of ours answers: go2rtc dialled
  it whenever no publisher was attached, and anything else that bound the port
  would have been served to viewers as that camera. Push cameras now get an
  unbindable placeholder instead; a camera that is genuinely pulled keeps its
  real serve URL.
...
- **Requires `python-aidot-cameras` 1.0.0rc27.** That release fixes the frame
  gap reporting on the direct-publish path (it could report a silence as
  dropped frames), stops a non-finite audio gain setting taking a stream down,
  and keeps two SDP file operations off Home Assistant's event loop - one of
  which Home Assistant was flagging as a blocking call.

## [2.29.0]

### Fixed

- **Direct publish no longer blanks standard (DTLS) cameras when go2rtc is
  the one bundled with Home Assistant.** That go2rtc is reachable by Home
  Assistant but not by this integration, which is exactly why those cameras
  are normally served on a local port that Home Assistant hands to go2rtc
  itself. With the option on they were instead pointed at an address nothing
  was listening on, and the live view stayed blank. Those cameras now switch
  to publishing only when this integration can actually reach go2rtc, and
  otherwise keep the local serve that works. Spotlight/PTZ and battery
  cameras are unaffected - publishing is their only path either way.

## [2.28.0]

### Fixed

- **Camera audio sounds like it used to with direct publish on.** That mode was
  sending a mains camera's audio exactly as the camera produced it, without the
  level correction the previous path applied - so a quiet camera stayed quiet
  and a loud one was never held back. The correction is back, and a silent
  camera no longer has its background hiss amplified.

### Changed

- **Requires library `1.0.0rc26`,** which also reports how long a live view
  went without a new frame, so a stall can be diagnosed from the log.

## [2.27.0]

### Changed

- **Direct publish takes H.264 cameras only.** A camera that negotiates H.265
  for a session keeps the previous streaming path for it, which has always
  carried H.265. The publisher is measured on H.264 across seven cameras, and
  a camera's H.265 has never gone through it - the spotlight/PTZ model picks
  its own codec and chose H.264 in 9 of 9 sessions when asked. Nothing changes
  with the option off, and nothing changes for H.264 cameras with it on.
- **Requires library `1.0.0rc25`.**

## [2.26.0]

### Fixed

- **Live view keeps its audio in the Picture-card dialog when direct publish is
  on.** That mode sends the camera's own G.711 audio, which Home Assistant's
  HLS player cannot carry, so the dialog played video with no sound. go2rtc now
  also gets a transcoding source for those cameras, listed after the live one:
  a player that needs AAC gets it (started on demand, stopped with its last
  viewer), and WebRTC cards keep the untranscoded audio. Nothing changes with
  the option off.
- **Requires library `1.0.0rc24`.**

## [2.25.0]

### Added

- **Direct publish into go2rtc (experimental option, off by default).** A new
  "Direct publish into go2rtc" switch in the streaming options turns on the
  library's in-process RTSP publisher (`AIDOT_DIRECT_PUBLISH`): no ffmpeg
  process per stream, no audio transcode for live view. With it on, DTLS
  (A000088) cameras publish into go2rtc too, the way SDES cameras already do,
  instead of being pulled from a local serve port - so they wait on the
  publisher reaching go2rtc rather than on a port. The HLS fallback plays video
  without audio in this mode. A library without direct publish leaves DTLS
  cameras on the pull serve.
- **Requires library `1.0.0rc23`.** That release carries the publisher this
  option switches on, plus a fix for the standalone `aidot-go2rtc` CLI's
  stdout mode, which served no media.

## [2.24.5]

### Fixed
- **A clear message when you tap a listed-but-not-playable recording.** SD recordings are listed for reference, but Home Assistant cannot pull a clip off the camera's card - only the AiDot app can. Tapping one (or any non-playable listing row, such as "No cloud recordings") used to fail with a cryptic `Invalid identifier: ...`. The media browser resolves any tapped row whether or not it is marked playable, so those rows now explain themselves: an SD clip points you to the AiDot app, and other listing rows say they are a listing, not a playable recording.
- **Requires library `1.0.0rc22`.** Its only change is observability - a cloud-call timeout now logs an identifiable exception instead of a blank line - so nothing you use behaves differently.

## [2.24.4]

### Fixed

- **A clip tapped seconds after its motion push no longer shows "clip transcode
  failed".** A motion push fires the instant motion is detected, but the cloud
  takes a few seconds to finish the recording, so a tap right away reached the
  clip proxy before the playback URL existed. The proxy returned a bare 502
  "clip transcode failed" for that - though no transcode was even attempted -
  the same response it gives when ffmpeg genuinely fails. Now the proxy waits a
  bounded few seconds for the clip to become available (so the common case just
  plays), and if it still is not there answers 503 "clip not ready yet, try
  again shortly" with Retry-After, distinct from a real transcode failure
  (still 502). Tunable with AIDOT_CLIP_READY_RETRIES / AIDOT_CLIP_READY_DELAY_S.
- **Downloading diagnostics no longer logs "Detected blocking call ... inside
  the event loop".** The library version lookup reads package metadata from
  disk; it now runs in Home Assistant's executor instead of on the event loop.
- **Requires library `1.0.0rc21`.** Its fixes reach you here as: a battery
  camera that answers and then goes quiet is retried after about 20 s instead
  of leaving the live view spinning for 75 s, and a DTLS camera session that
  never delivers a keyframe is now treated like one delivering no video at all,
  so the existing give-up-and-retry handling applies to it instead of it
  counting as a working stream that shows nothing.

## [2.24.3]

### Fixed

- **A failed hub-id stash can no longer take the whole entry down.** `2.24.1`
  caches the account hub's registry id during setup so entities can nest under
  it on Home Assistant 2026.9. The assignment was guarded with try/except, but
  the handler called a logger that module never defined - so the guard would
  have raised `NameError` out of setup *before* any platform loaded, taking every
  aidot entity with it. The exact outage it was written to prevent.

  Unreachable on today's coordinator, and caught by review rather than a user;
  the repository's own lint gate had been flagging it since `2.24.1` while the
  test suite stayed green. A test now drives setup with a runtime object that
  refuses the stash and asserts the platforms still load.

- **Requires library `1.0.0rc20`.** Its fixes reach you here as: snapshots on
  an SDES camera are bounded again (`rc19`'s arrival stamping had silently
  defeated the bound), and a session the cloud declares void now re-logs in
  directly instead of first spending a refresh attempt that could leave the
  entry stuck.
...
- **Requires library `1.0.0rc19`, which stops a camera's own backward
  timestamps from reaching the muxer.** The battery L2 emits a packet that is
  the next in sequence, that nobody asked to be resent, and whose timestamp is
  ~1.7 s in the past - on a 30-second period. The serve now stamps by arrival
  instead of trusting the camera's clock.

  Measured on a real install, controlled five-minute views:

  | | before | after |
  |---|---|---|
  | backward-DTS warnings | 6.1-6.8 per streaming-minute | **0** |
  | video vs audio start | 2.7-3.9 s apart | **0 ms** |
  | decode errors | unchanged | unchanged |

  What you should notice: recordings and HLS on a cold battery camera were
  liable to fail with a muxer error about non-monotonic timestamps, and
  shouldn't now. Live view was never affected - ffmpeg was already absorbing it
  - but audio and video now start together instead of video lagging by seconds.

## [2.24.1]

### Fixed

- **Home Assistant 2026.9 took out 50 entities across 11 domains, six cameras
  among them.** 2026.9 removed `via_device` from `DeviceInfo` and turned passing
  it into a `RuntimeError` raised from inside
  `device_registry.async_get_or_create`. It surfaces as
  `Error adding entity None for domain camera with platform aidot`, and the
  entity is never added at all - so cameras, switches, sensors and the rest
  simply read *unavailable* after upgrading.

  The link to the AiDot hub device now uses whichever spelling the running
  Home Assistant declares. The two are not interchangeable: `via_device` takes
  an identifier tuple, `via_device_id` takes the hub's registry id, so the id is
  captured at setup where Home Assistant already returns it.

  If the hub cannot be resolved on a new Home Assistant, the device links to
  **nothing** rather than falling back to the key that raises. Losing the
  grouping is cosmetic; losing the entity is not.

### Changed

- **A guard against the next removal like it.** A test now refuses to emit any
  `DeviceInfo` key the *running* Home Assistant does not declare, whichever
  version that is.

  Upgrading the test pin could not have caught this and still cannot: the
  newest `homeassistant` on PyPI is 2026.2.3, while Home Assistant OS ships
  2026.9.x - so the version that broke here cannot be installed in CI at all.
  CI now prints the Home Assistant it actually tested against on every run, so
  that gap is visible in the log rather than assumed from the job name.

## [2.24.0]

### Added

- **Light behavior** -- whether the floodlight comes on **Constant** or
  **Flash** when it triggers. The third setting under *Light when someone
  appears*, and the one that was missing.

  It was held back in 2.23.0 because the write was acknowledged and the value
  never changed. That turned out not to be about the setting at all: a battery
  camera was never given time to wake before the command arrived behind its own
  wake. Fixed in library `1.0.0rc18`, which this release requires, and the
  write now lands on a sleeping camera -- verified by reading the value back off
  the camera, which is the bar a control has to clear here.

  Gated on the **model declaring the setting**, like the duration, so it appears
  on the battery L2 line and not on the M3 Pro line. A value the camera reports
  that is not one of the two offered reads as *unknown* rather than being shown.

### Fixed

- Battery cameras dropped control commands generally, not just this one. The
  wake and the command were sent together, so a sleeping camera was still coming
  up when the command arrived. Any setting on a battery camera should be more
  reliable on this release; that fix is in the library.

## [2.23.1]

### Fixed

- **The "Light stays on for" control appeared on cameras that ignore it.**
  2.23.0 offered it wherever the cloud reported a value. On an A000088 the
  cloud reports `LingerDuration: "30"` while the camera, asked directly on the
  LAN, has no such setting and silently discards the write. Three of the five
  controls on a seven-camera fleet were therefore controls that could never
  work.

  It is now gated on the **model declaring the setting** in its product
  profile, which is what the vendor app gates on. In practice that means the
  duration control appears on the battery L2 models and no longer on the M3
  Pro line.

  If you installed 2.23.0, the stale control is **removed from your entity
  registry** on the next start rather than being left behind as a permanently
  unavailable entity. Removal is keyed on the model not supporting it, never on
  a camera simply not having answered yet.

- **Light brightness is deliberately unchanged.** The A000088's profile omits
  `Dimming` too, but the camera really has it: a LAN write was applied by the
  camera and echoed back by the cloud. Gating that one on the profile would
  have removed a control that demonstrably works, so it stays gated on the
  reported value. The asymmetry is evidence-based, and both halves are pinned
  by tests.
...
- **The settings under "light when someone appears".** The switch that turns
  the camera's floodlight automation on has shipped for a long time -- the
  vendor app calls it "When Someone Appears". Two settings that sit under it in
  the app now have controls of their own:

  - **Light stays on for** -- 20, 30, 40 or 50 seconds. Exactly the four the
    camera offers; it is not a free range.
  - **Light brightness** -- 10 to 100. The floor is **10, not 0**: zero is
    outside the range the camera accepts, and "off" is the floodlight itself.

  Both only appear on cameras that actually report the setting. A camera that
  reports neither gets neither entity, rather than two controls stuck at
  *unknown*.

### Changed

- The floodlight-automation switch is now called **Light when someone appears**,
  which is what it does. Its entity ID is unchanged, so existing automations and
  dashboards keep working.

### Not added, on purpose

- **No light-behaviour control** (Constant / Flash). The camera reports the
  setting and the vendor app offers it, but the write is acknowledged and the
  value never changes -- six attempts across two models, every read-back
  unchanged, while the two settings above landed within four seconds on the same
  camera in the same session. A control that reports a setting the camera never
  applied is worse than no control, which is the same reason there is no
  resolution select.
...
- **Automatic siren controls.** Three new switches per camera: **Automatic
  siren**, and whether it fires **on motion** or **on a person**. Separate from
  the existing siren entity, which sounds it right now -- these decide whether
  the camera does so by itself.

  They are named for the consequence rather than the setting, because turning
  the first one on is a decision to let a siren go off in your house. The master
  and the two triggers are independent entities, and changing a trigger cannot
  arm the master.

  A camera that has not answered yet shows **unknown**, not *off*. A confident
  "off" would read as "the siren will not sound" without the camera having said
  so. Battery cameras are asleep and answer nothing here, as with the other
  camera settings.

  Requires library `1.0.0rc15`.

## [2.21.1]

### Fixed

- **Requires library `1.0.0rc14`,** in which every camera session now tells the
  camera what kind of client it is. One character of the session identifier
  carries a client class, and the library had been filling it with a random
  value -- announcing a class the vendor does not define eleven times in
  sixteen. It now announces the same class the vendor's own web app does.

  Nothing you can see changes. It was screened on real cameras before shipping,
  including under the conditions that broke an earlier attempt at this, and the
  class involved is not one that alters how the camera manages a session.

## [2.21.0]

### Added

- **Your cameras can tell a person from a car from a parcel from a pet, and now
  Home Assistant can too.** Four new switches per camera -- **Human detection**,
  **Vehicle detection**, **Package detection** and **Pet detection** -- under
  the camera's device page.

  Until now this integration offered motion on/off and a sensitivity number,
  while the camera was already doing the harder work: on a stock M3 Pro and PTZ,
  human detection reads as *on* out of the box. That distinction never reached
  Home Assistant, so an automation could not say "only when it is a person" from
  the camera's own detector.

  The switches appear once the camera has answered, like the sound detectors --
  it is a slow round trip, not a cloud attribute. A camera that has not answered
  shows **unknown**, not *off*: battery cameras are asleep and answer nothing
  here, and four switches confidently turned off would be a state the camera
  never reported.

  Requires library `1.0.0rc13`.

## [2.20.3]

### Fixed

- **Requires library `1.0.0rc12`, which stops the integration flooding your
  Home Assistant log.** Home Assistant's own rate limiter was firing against
  it -- twice in ten minutes on an idle system with three cameras connected.

  The library reports a session's progress through a channel that was built for
  its command-line tool: when something is showing those messages on a screen
  they are logged quietly, and when nothing is, the log becomes the channel and
  they are logged at INFO. Running inside Home Assistant, nothing ever shows
  them, so per-packet diagnostics ended up at the same level as the handful of
  lines that describe a session starting. Measured over 11.7 hours on a live
  system with eight cameras, INFO lines from the library fall from 39584 to
  437 -- from roughly 3400 an hour to 37. The lines that describe a session are
  deliberately kept.

  If you turned this integration's logging down to `WARNING` to escape the
  noise, you can put it back to the default.

- **A camera's decrypted audio is no longer written to your log in the clear.**
  One diagnostic dumped a packet's full decrypted contents as hex at INFO. It
  is now bounded to a short prefix.

- **A failed camera connection now reports which transport failed.** The
  diagnostic that explains a connection failure could stop at the first
  transport it could not read -- hiding the rest, in a report whose whole
  purpose is finding the broken one -- and could raise an exception of its own
  into the WebRTC stack while doing it.

## [2.20.2]

### Fixed

- **Requires library `1.0.0rc11`,** which corrects how every camera session
  identifies itself during connection setup. The offer carried a separate ICE
  username and password for each media track, but only the first pair survives
  into the message the camera actually receives -- so the camera held the audio
  pair while the video socket kept signing with the video one, a password the
  camera had never been given. Nothing looked broken, because the cameras
  tolerated it, but it was wrong on every session on the SDES path. The offer
  now carries one pair for the whole session, which is what the camera holds and
  what the vendor's own app sends. The credentials also use the character set
  the standard defines; the previous ones put an out-of-spec character into
  about half of all passwords.

- Carries `1.0.0rc10` as well, which was released after `2.20.1` pinned `rc9`.
  A camera that cannot stream at all -- one that has dropped off the WiFi, say
  -- now abandons the attempt instead of launching an ffmpeg that immediately
  fails, measured at 11 attempts and 6 stalls in 25 minutes before the fix.
  Press-to-talk, the siren and snapshots are explicitly never abandoned by this.
  A battery session no longer loses its audio when the first packets arrive a
  fraction of a second after the wait ends, and a malformed
  `AIDOT_ABANDONED_MEDIA_GRACE_S` no longer stops the integration loading.

## [2.20.1]

### Fixed

- **Requires library `1.0.0rc9`,** which closes three issues found reviewing the
  connect-latency work that `2.20.0` shipped. The shortened wait for the
  broker's echo is now per camera -- a camera seen to echo goes back to the
  original two seconds on every later view, so a camera whose echo is merely
  slow cannot quietly lose the branch that echo drives. The broker's echo of our
  own answer is no longer mistaken for the camera's, which could leave a session
  trying to connect to itself. And two views of one camera opening at once no
  longer disturb each other's handshake.

- The cold-start figure for SDES cameras in the README said 6.7 s; it is about
  2.5 s since `2.20.0`.
...
- **A camera on the SDES path opens in about 2.5 s instead of 6.7 s.** All of
  the work is in the library (`1.0.0rc8`), which this now requires; the
  integration change is the requirement itself.

  Two waits on the connect path were spending time on things that had already
  happened. The ICE responder window was written when the camera's answer
  arrived after it closed, and an earlier speed-up inverted that -- so the
  window sat for seconds holding the credentials it was waiting for. And the
  wait for the broker to echo our own request back was a flat 2 s that, across
  18 hours of one deployment, expired every single time without the echo ever
  arriving.

  Measured on the reference PTZ over five consecutive cold opens: time to first
  media 6702 ms -> 2489 ms, and 2695 ms over eight opens of the shipped build.
  Battery cameras and the DTLS models take neither wait and are unchanged.

## [2.19.12]

### Fixed

- **A battery camera's first view failed about half the time.** The integration
  handed Home Assistant the stream URL after 8.5 s whether or not the camera was
  ready, and a cold battery open attaches its publisher anywhere between 5.5 s
  and 11 s -- so on the slow half the card asked go2rtc for a stream that had no
  publisher yet, got a connection refused, and stopped. That 8.5 s exists to fit
  Home Assistant's 10 s limit on the HLS path, and that path is the only caller
  under it: the WebRTC provider and third-party cards have no limit at all. The
  HLS path keeps its budget and every other caller now waits long enough for the
  camera.

- **Half a second of dead time on every cold view.** The wait for the camera's
  publisher ended on a one-second poll, so it regularly sat there after the
  publisher had already arrived. It now polls four times a second.

- **A battery camera's first view could still fail while the camera was seconds
  from streaming.** A cold battery open is mostly the camera waking, and that
  wake is wide: measured on one camera in one day it announced itself +0.9 s,
  +5.4 s, +5.9 s, +10.9 s and +27.9 s after the request. Most of those reach the
  card in 8-13 s; on the slowest, media arrived at +46.7 s and the view had
  already given up 1.7 s earlier. Battery cameras now get a budget sized by that
  wake rather than by the handshake, so the view waits -- showing Connecting and
  then Buffering -- instead of failing. Mains cameras keep the short budget and
  still fail fast when they are genuinely unreachable.

### Changed

- Requires `python-aidot-cameras` 1.0.0rc7, which shortens a battery camera's
  first connect: it keeps a waking camera awake through its own handshake
  instead of letting it fall asleep partway, gives up on a stalled attempt in
  favour of the retry that works, and removes 1.25 s of dead waiting from every
  open of a PTZ or battery camera.

## [2.19.10]

### Fixed

- Clip links inside notifications now survive a Home Assistant restart. The
  link-signing key used to live only in memory, so every restart invalidated
  the links in notifications already delivered; it is now kept in Home
  Assistant's storage and links stay valid for their full lifetime (a week
  for notification links, six hours for Media-browser ones).

## [2.19.9]

### Fixed

- **Camera extras stopped updating, permanently.** Home Assistant eager-starts
  background tasks, so a fetch that finished without ever suspending -- an older
  library wheel with no such getter, or no live session -- ran its whole body,
  including its `finally`, inside `async_create_background_task`. The in-flight
  slot and the task handle were the same variable, so that `finally` cleared it
  and the already-finished task was then assigned back over it. From that moment
  every poll read "one already in flight" and no camera fetched Wi-Fi, SD-card or
  sound-detection details again for the life of the config entry. The guard is
  now claimed before the task is created and is never written afterwards.

- **A refused sound-detection write looked like a successful one.** The switch
  discarded the setter's return value, but the library returns `False` -- writing
  nothing -- when the camera does not answer `soundAlgorithmGet` or does not
  report the requested key. The switch moved, the read-back quietly reverted it,
  and no error was ever shown. A refusal now raises, like every other switch here.

- **One sound-detection toggle could block every switch on the account for over
  a minute.** The read-back after a write re-fetched all three extras at a 25 s
  timeout each, inside a service call on a platform that serialises them. It now
  asks only for the key the write changed.

- **A camera that answers nothing polled six times too often, forever.** The fill
  cadence counted stored values, and a `None` answer stores nothing -- so a model
  that does not implement one of these actions never completed the set and stayed
  at the fill rate for good. A key now counts once it has been asked for.

- **A config-entry reload left a fetch running against a dead coordinator**,
  holding it and its device client alive and then writing to them. The task is
  cancelled on unload; background tasks are otherwise only cancelled at Home
  Assistant shutdown.

- **An error inside entity creation vanished silently.** A `try/except
  AttributeError` meant to tolerate a coordinator without listener machinery also
  swallowed anything the listener callbacks raised, so a platform that hit an
  unexpected shape simply never created its entities, with nothing in the log.

- **Motion prewarm pushed to a stream go2rtc had not accepted.** The registration
  call returns `None` without raising when go2rtc is disabled or the request
  fails, and pushing anyway aimed RTSP at a name go2rtc did not know: the publish
  was refused and the ffmpeg and retry loop churned against a camera nobody was
  watching. Prewarm now stops instead.

- **Seven sound-detection switches were untranslatable**, named from a hardcoded
  English table instead of a translation key, and an unrecognised detector was
  rendered "Smoke t3". They use translation keys now.

- **`strings.json` had no names for three sensors and four switches** that
  `translations/en.json` did have, and listed two entities that no longer exist.
  CI only checked that the files parse; a test now checks that every declared
  translation key has a name in both files and that the two agree.

- **The three new diagnostic sensors were enabled by default** while every other
  camera diagnostic here is opt-in, so the Wi-Fi network name went into the
  recorder and diagnostics downloads without being asked for.

### Changed

- Requires `python-aidot-cameras>=1.0.0rc6`, for the reply matcher that treats an
  echoed `seq` as a preference rather than a requirement -- without it a camera
  that does not echo it reads as silent, which is the same read path the
  sound-detection fixes above are about.

- Corrects two claims about 2.19.8 below: battery cameras get no sound-detection
  switches at all rather than switches reading "unknown", and the Wi-Fi network
  sensor is named to match the existing Wi-Fi signal sensor.

## [2.19.8]

Requires python-aidot-cameras 1.0.0rc5.

### Added

- **Sound detection switches.** The cameras can listen for glass breaking, a
  smoke alarm, a baby crying and a dog barking, and none of it was visible in
  Home Assistant. Each detector a camera reports becomes its own switch, named
  from the camera's own key list rather than a fixed table, so a model that
  reports something new still gets a switch. Mains cameras only -- reading these
  costs an MQTT round trip and a battery camera would have to be woken for it,
  so those get no sound switches rather than switches that never resolve. A
  mains camera that has not answered yet reads as unknown rather than off, since
  a row of off switches would claim a state it never reported.
- **Wi-Fi network, SD card total and SD card used** as diagnostic sensors,
  disabled by default like every other camera diagnostic here -- the network
  name in particular should not land in the recorder unless somebody asks for
  it. The SD figures carry no unit: the camera reports bare numbers whose units
  are unconfirmed, and declaring bytes would render a number that looks
  authoritative and could be wrong by a factor of a million.

These come from the camera itself rather than the cloud device record, so they
refresh on a slow cadence (half an hour) instead of the five-minute attribute
poll, and battery cameras are never woken for them.

## [2.19.7]

Requires python-aidot-cameras 1.0.0rc3, which fixes late-joining viewers being
spliced onto audio instead of a video keyframe. An existing install already
satisfied the previous requirement, so the pin moves to make sure the fix
actually reaches it.

### Fixed

- **Push cameras no longer register a go2rtc producer on a port nothing binds.**
  In SDES push mode nothing ever binds the local http serve - the media is
  published into go2rtc over RTSP instead - so registering that serve at setup
  created a stream whose only producer was a dead port, for every camera,
  viewed or not. A viewer's offer then died on it with
  `dial tcp 127.0.0.1:189xx: connect: connection refused` while the card sat on
  "Loading..". The stream definition is now created only immediately before a
  publish, so the placeholder never outlives a publisher that is on its way.

## [2.19.6]

Requires python-aidot-cameras 1.0.0b34, which fixes the A001064's streaming
interruption at its cause and makes SD card listing work on SDES cameras.

The library never sent an SCTP acknowledgement, so the camera's control channel
retransmitted until it gave up and tore the association down about a minute into
every session. Its own keepalive watchdog then ended the whole session twenty
seconds later. That is the roughly-every-two-minutes interruption on the PTZ, and
it is also why PTZ control, two-way audio and on-device recordings stopped
answering after the first minute of a view - they all ride that channel.

### Fixed

- **An empty on-device folder no longer blames the camera model.** The message
  said "some models do not report their card contents", on the strength of one
  camera that answered nothing to five requests. The camera had replied; the
  reply arrives in several fragments and the library discarded them. No model on
  the reference fleet is known to lack these commands, so the message no longer
  sends anyone looking for a limitation that may not exist. "The camera did not
  answer" now means what it says.

## [2.19.5]

No functional change from the final 2.19.4. This exists only so the fixes below
actually reach installs.

2.19.4 was republished in place after two further changes - the clip now rides
in the notification, and the inline-header note was corrected. HACS decides
whether to offer an update by comparing version strings, so anyone who installed
2.19.4 during the window before the republish would have been left on the
earlier build with no update ever offered. A new version number is the only
thing that reaches them.

If you installed 2.19.4 after the republish you already have this code; the
upgrade is a no-op.
...
- **The clip now plays inside the notification itself.** Motion notifications
  carry the event clip in the companion app's `video` field, so the app pulls
  frames from it and animates them in the notification - the clip is visible
  without opening anything. Tapping still opens it full size.

### Changed

- Event clips are served with `Content-Disposition: inline`, the correct header
  for content meant to be played rather than saved.

  This was first written up as fixing a download-instead-of-play bug on
  Android. That was wrong, and the entry is corrected rather than left standing:
  the companion app's `createOpenUriPendingIntent` only hands a URL to the
  system (where a download could happen) when it is ABSOLUTE. The link in these
  notifications is relative, so the app navigates its own frontend to it and
  never takes that path. The header is right on its own terms and is kept; it
  was not fixing the symptom claimed.

## [2.19.3]

### Fixed

- **A motion notification that never arrived was recorded as sent.** The
  dispatcher called the notify service without waiting for it, so the platform's
  actual failure - a stale companion-app registration, a revoked token, an
  unreachable push service - happened afterwards where nothing observed it. The
  send is now awaited, so a failure is logged and the "no notification target"
  warning is no longer cleared on the strength of a delivery that was never
  attempted. For a feature whose entire value is that the message got through,
  silent total failure was the worst possible mode.

- **Clip links in notifications expired after 6 hours.** That window was chosen
  when only the media browser minted them, where a link is tapped moments after
  browsing. Notifications mint the same link when the motion happens, so one
  arriving at 23:00 and tapped at 08:00 was rejected. Links inside notifications
  now last a week. Replay is still bounded: the signing key is regenerated on
  every restart, so no link outlives one.

- **An entry that had turned off SDES serve mode was stuck on the mode that
  jams.** 2.19.0 removed the toggle from the options page because publishing
  into go2rtc is the mode that works - but the camera platform kept reading the
  stored key, so anyone who had switched it off in 2.18.x stayed on the pull
  serve with no way left to change it. The stored value is now ignored.

## [2.19.2]

### Fixed

- **Requires library 1.0.0b33, which recovers every H265 session on an
  A001064 PTZ.** The library used to demux a camera's media by a fixed
  payload-type table (96/97/98 video, 0/8 audio). Measured across 107 opens
  on one camera, 15 of them (14%) negotiated H265 on payload type 0 and said
  so in the answer, then sent video on pt=0 while audio ran on pt=8 - so the
  fixed table posted the whole video stream to the audio side. Those sessions
  produced no picture, spent 82 seconds discovering it, and then reopened.
  b33 demuxes by the answer instead. Verified on hardware across seven H265
  sessions, every one of which now reaches the serve in about 11.5 seconds
  instead of failing at 82.

  Scope of that claim: it restores the session and produces a real H265 stream
  where there was none. Whether a given player renders H265 is a separate
  question this does not change - H265 playback support varies by browser and
  by pipeline, and no H265 session has yet been watched end to end through a
  viewer.

  The floor is raised rather than left at `>=1.0.0b32` because b32 already
  satisfies that specifier, so an existing install would never upgrade and
...
- The streaming options page is titled "Streaming and connection", matching
  its menu entry.
- The notification target list now offers the legacy `notify.notify` group
  target alongside the other notify services.
- The mirror `strings.json` carries the same connection-mode wording as the
  rendered `translations/en.json`.

## [2.19.0]

### Added

- **Motion push notifications.** The options page now has a menu. Under
  **Motion notifications - targets and wording** choose the notify services to
  send to (any `notify.*` service, for example a phone running the companion
  app; you can also type a service name), a per-camera cooldown, and the title
  and message. Under **Motion notifications - per camera** pick a camera and
  choose Off, All motion and person events, or Person only, with an optional
  list of targets that overrides the defaults for that camera. Placeholders
  for the wording: `{camera}`, `{event}`, `{event_title}`, `{time}`,
  `{device_id}`.

  Each notification carries the event's still image, and tapping it opens the
  clip through the integration's own player. Repeated notifications for the
  same camera replace each other instead of piling up. A person detection is
  delivered even inside a cooldown that plain motion started.

  Changing any notification setting takes effect on the next event without
  reloading the integration, so live streams are not interrupted by it.

  Known limitation: the clip link inside a notification is signed with a key
  that is renewed each time Home Assistant starts, so after a restart the
  links in notifications delivered before it stop working (the still image
  and the notification itself are unaffected). Open the clip from the Media
  browser in that case.

### Removed

- The **SDES adaptive fast-with-fallback** option. It was experimental, off by
  default, and a failed fast attempt cost about 40 seconds before the fallback.
  The library keeps its own environment switch for anyone who wants to test it.
- The **SDES serve mode** toggle is no longer on the options page. Publishing
  into go2rtc is the mode that works; the pull serve it could switch to is the
  one that jams under Home Assistant. An entry that had turned it off keeps
  its stored setting.
...
- **Library floor raised to `python-aidot-cameras >= 1.0.0b32`**, which makes
  live streams survive a camera that briefly stops transmitting:
  - The serve now tolerates up to 30 seconds of input silence on mains
    cameras (battery cameras keep the fast 10 second default - their silences
    are real sleeps that only a reopen ends), so a transmit gap the link
    recovers from no longer kills the stream outright.
  - When media that had started goes silent, the bridge re-sends the session's
    start trigger a few times and requests an immediate keyframe on resume,
    so a stream that can recover in place does so without a cold reopen.
  - The existing watchdog-and-reopen path is unchanged and remains the
    fallback, so worst-case behavior is no worse than before.

  Measured on a pan-tilt model that periodically drops off the network for
  about a minute at a time: recovery now takes as long as the camera's own
  outage and no longer compounds it. A floor that merely permits a version
  never installs it, so this release exists to actually deliver the library
  update to existing installs.

### Added

- **Connection mode is now honest, plumbed, and verifiable.** The option's
  default was labeled "Relay" while actually meaning "either path, LAN
  preferred" - it is now labeled **Automatic - LAN preferred, relay
  fallback**, which is what it always did: every camera that can reach Home
  Assistant directly streams over the LAN, and only a camera with no direct
  route rides the cloud relay (verified on a seven-camera fleet: six direct,
  one relayed). LAN only keeps its meaning, and battery cameras keep the
  relay regardless - it is their only path back from a cloud wake. The choice
  now also travels to the streaming library as `sdes_connection_mode`.

- **Diagnostics reports `media_path` per camera** - `direct` or `relay`, the
  path the live session's media is actually taking, stamped from the first
  packet's source address. The option states a preference; the camera decides;
  this is the receipt. A label, not an address, so nothing new to redact.

- There is deliberately **no relay-only option**. It was built and measured:
  even with the offer's connection address moved to the relay allocation and
  the TURN permission pre-installed, both camera families still dialed the
  direct path (the firmware learns our address from ICE probes and replies to
  wherever packets came from). An option that says relay-only while sessions
  run direct is a control that lies - the standard that removed the
  resolution select in 2.11.9. The library keeps an experimental env knob
  with the limitation documented.

Requires library 1.0.0b31 (`sdes_connection_mode`, `media_path`).
...
- **Streams no longer churn for the first minutes after an update or core
  restart.** The library caches each camera's H.264 decoder parameters and
  injects them when a stream starts, so playback does not have to wait for an
  intact keyframe; that cache lived in the container's home directory, which
  is wiped every time the container is recreated. After each update, every
  camera's first stream start ran without the cache, and cameras on weak
  wireless links could take several attempts to come up - visible as cards
  going blank and reconnecting on live views right after a restart. The cache
  now lives under the Home Assistant config directory and survives updates.
  An explicit `AIDOT_SPROP_DIR` environment setting still wins.

## [2.17.10]

### Changed

- **Requires library 1.0.0b30.** Carries the credentials fix (a configured
  country other than US now reaches the cloud login on the password path,
  and the login password is no longer exported to spawned helper processes)
  and a reviewed quality pass over the camera-feedback internals: the
  retransmission, keyframe-request and bandwidth-feedback paths now share
  one send implementation, and their bookkeeping costs the packet loop less.
  No user-visible behaviour change is intended; updating puts the reviewed
  code into service ahead of the 1.0 release candidate.

## [2.17.9]

### Changed

- **Two expected-behaviour notes in the README's troubleshooting section**,
  so they reach HACS installs: a battery camera's first view after a long
  sleep can fail once and succeed on the automatic retry, and a camera
  removed at the vendor stays registered (unavailable) until deleted in
  Settings -> Devices & Services. Documentation only; no code change.

## [2.17.8]

### Changed

- **Examples and test fixtures use neutral names and synthetic addresses.**
  The README's card example, one changelog line, and a handful of test
  fixtures referred to specific devices and addresses from the reference
  installation. They now use generic names and reserved private addresses.
  No code change; the diagnostics redaction these fixtures exercise is
  unchanged and re-verified by the same tests.

- The note explaining why there is no resolution/quality select entity now
  carries the 2026-08-23 bitrate measurement alongside the original
  dimensions evidence: the command is acknowledged and has no effect on
  either codec the camera produces, so a control would report a setting the
  camera never applied.

## [2.17.7]

### Changed

- **Camera control messages now match the vendor app byte for byte.** Requires
  library 1.0.0b27.

  One field in the message header was being filled with a random number where
  the app uses a running counter tied to the live view. Everything else already
  matched. No behaviour is expected to change -- pan, tilt and two-way audio
  worked before and were re-tested on hardware after the change -- but it
  removes the last known difference between what this integration sends and
  what the app sends, which matters when diagnosing controls the camera
  acknowledges and then ignores.

## [2.17.6]

### Fixed

- **Fewer stalls on a camera with a weak signal, continued.** Requires library
  1.0.0b26.

  When packets go missing the camera is asked to resend them. On a link that is
  already carrying as much as it can, a burst of those requests costs more than
  it recovers -- measured on the affected camera, loss rises steadily with how
  much the camera is sending, from 0.4% up to 7%. The requests are now spaced
  out so a burst of losses cannot make the moment worse. Nothing is asked for
  less; it is asked for in fewer, fuller messages.
...
- **A camera could show a connected stream with no picture.** 2.9.8 began passing
  ...
  media. Seen on the A001064 PTZ, whose stream had two producers, one of them
