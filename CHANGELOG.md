...
- **Sound detection switches.** The cameras can listen for glass breaking, a
  ...
  reports something new still gets a switch. A camera that has not answered
  reads as unknown rather than off -- battery cameras answer none of this while
  asleep, and showing a row of off switches would claim a state they never
  reported.
- **WiFi network, SD card total and SD card used** as diagnostic sensors. The
  SD figures carry no unit: the camera reports bare numbers whose units are
  unconfirmed, and declaring bytes would render a number that looks
