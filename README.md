...
### Experimental: direct publish into go2rtc

**Settings -> Devices & Services -> AiDot -> Configure -> Streaming ->
Direct publish into go2rtc** (off by default). With it on, each camera's
video and audio go straight into go2rtc from the integration's own process -
no ffmpeg process per camera and no audio transcoding - and the standard
(DTLS) models publish the same way the SDES models already do instead of
being served on a local port. In side-by-side tests on seven cameras the
stream reached go2rtc sooner on every one, most of all on the standard
models (about 7 s -> under 2 s), with no packets lost.
...
Live WebRTC views carry the camera's own audio (G.711) with no transcoding.
A player that needs AAC - Home Assistant's own HLS dialog, the one the Picture
card opens - is served a transcoded copy instead, started on demand and stopped
with its last viewer, so audio works on both.
...
To serve WebRTC, each camera is registered with go2rtc as a stream named
`aidot_` plus the first twelve characters of its device id. Once it is there,
anything that can reach **your go2rtc** can pull that camera - over RTSP,
WebRTC or HLS - and go2rtc does not ask for a Home Assistant login:
...
without a second cloud connection or a second slot on the camera, which only
serves one viewer at a time.

It is also the whole exposure, so be clear about it:

- **There is no per-stream authentication.** Anyone who can reach go2rtc's
  RTSP or API port can list the streams and view any camera on them. Verified
  on a live install: from a different machine on the same LAN, with no
  credentials, `GET /api/streams` answers and every camera pulls.
- **How far that reaches is go2rtc's configuration, not this integration's.**
  Home Assistant's bundled go2rtc and a standalone one have different defaults,
  and go2rtc binds its RTSP port on all interfaces unless told otherwise. If
  your network is not one you would hand a camera feed to, restrict go2rtc -
  bind it to loopback, firewall the port, or turn on its own authentication.
- **A stream only carries media while the camera has a live session.** With the
  ...
  warm-hold, or on a battery camera, it carries media while someone is
  watching and for the warm-hold window after.
- **The integration's own local serve is loopback only** (`127.0.0.1`), and
  the library warns if decrypted media is ever served on a non-loopback
  address. go2rtc is the part that reaches the network.

None of this is new in a recent release - registering the camera with go2rtc
is how WebRTC has always worked here. It had simply never been written down.
