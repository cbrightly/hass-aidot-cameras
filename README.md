...
- **Integration keeps reloading / motion + occupancy look stuck** - self-reloading
  ...
  `...` menu).
- **Camera connects but the picture stays blank (some battery / PTZ cameras)** -
  a few SDES cameras (battery L2 models and the A001064 PTZ) finish connecting but
  never send video when their media cannot get back through the cloud relay -
  common when the camera is isolated from Home Assistant on the network (a
  separate VLAN, or AP/client isolation). The view then times out blank. If the
  camera shares the Home Assistant LAN, set **Connection mode** to **LAN-direct**
  to bypass the relay; otherwise it is a camera/network limitation, not a bug. The
  repeated `ffmpeg SDES stderr` log warnings this used to produce are quieted as
  of **2.8.9**. A blank picture on **mains (M3 Pro)** cameras was a separate bug
  fixed in **2.9.14** - update first.
