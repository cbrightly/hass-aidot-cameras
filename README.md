...
1. **Use a WebRTC dashboard card, not the default Picture / Picture Glance
   ...
   That dialog is also silent - see [Audio](#audio) below - so a WebRTC card
   is what you want for sound as well as speed.
...
the Picture and Picture Glance cards open - does not.**

That dialog only accepts AAC, so go2rtc used to transcode a copy of the
camera's G.711 audio for it. That copy arrived with stretched timestamps: a 15
second recording claimed to be 163 seconds, and because players sync to audio,
the dialog played in slow motion on every camera and every model. Dropping the
copy is what fixed the speed, and losing sound in that one dialog is the price.
A stream at the right speed without audio beats one at a tenth speed with it.

So if you want sound, use a WebRTC card. Two-way audio (talking *to* the
camera) is unaffected and works regardless of the card.
...
- **Live view plays in slow motion, roughly a tenth of real speed.** Fixed in
  **2.30.4** - update. The Picture card's HLS dialog was being served an AAC
  ...
  camera and every model. That dialog is now silent and plays at the right
