...
live:
  # Pin the go2rtc WebRTC path. Important, not just for speed: without it the
  # card's default `auto` provider can fall back to Home Assistant's HLS stream
  # pipeline, which on a multi-camera dashboard may fail to start a tile
  # (a spinning "connecting" that never resolves) while go2rtc is still warming
  # its pull. Pinning `go2rtc` keeps every tile on the WebRTC path.
  provider: go2rtc
...
Set `live.provider: go2rtc` on **every** card (one camera per card, or a multi-
camera card) — that's what keeps each tile on WebRTC and off the HLS fallback.
A grid of single-camera cards:
...
live:
  provider: go2rtc
  auto_play: [selected]
# …repeat one card per camera, each with `live.provider: go2rtc`
```
