...
|---|---|
| mains, standard models | **1.4 - 6.7 s** |
| mains, SDES models (PTZ / spotlight) | **0.9 - 2.1 s** |
| battery models, from idle | **~9 s** |

Battery cameras are deliberately not kept warm -- holding a session drains them --
so they pay the handshake on each view. Figures are one install on one network;
treat them as the shape to expect, not a guarantee.
