...
|---|---|---|
| **Fast connect (LAN-direct)** | On | Skips the relay/handshake waits so a camera on the local network starts in ~1 s instead of ~5 s. Turn off only if a camera is reachable from the Home Assistant host solely over the internet or through strict NAT (this mode uses no TURN relay). |
...
| **SDES camera audio (experimental)** | Off | Includes audio in SDES camera streams (PCMA transcoded to AAC). Off by default because battery and weak-signal cameras send audio too sparsely, which stalls the stream. Enable only for mains-powered cameras with a reliable connection. |
| **SDES fast connect (faster signaling)** | Off | Shortens the connection signaling for SDES cameras (battery models) by ~4.5 s, so their live view starts sooner. The PTZ camera is auto-excluded (needs the slower handshake). Validated in testing (a multi-open soak with no instability); kept off by default while it gets broader real-world use. If you enable it and ever see a live view drop to a snapshot, turn it back off. |
