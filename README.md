...
- **Cameras**
  ...
  - **Sound detection** -- the cameras listen for glass breaking, a smoke
    ...
    becomes its own switch. A camera that has not answered reads as *unknown*
    rather than off: battery cameras answer none of this while asleep, and a
    row of off switches would claim a state they never reported.
  ...
  - Diagnostics: battery, SD-card status, WiFi signal and network name, and
    SD-card total/used. The SD figures are shown without a unit on purpose --
