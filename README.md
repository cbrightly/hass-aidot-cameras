...
|---|---|
| mains, standard models | **0.6 - 1.9 s** |
| mains, SDES models (PTZ / spotlight) | **0.7 s warm, ~6.7 s cold** |
| battery models, from idle | **~8 - 13 s** |

Battery cameras are deliberately not kept warm -- holding a session drains them --
so they pay the handshake on each view. What they pay is almost entirely the
camera waking: the integration's own signalling is now under a second of it.
That wake is variable -- measured on one camera in one day it announced itself
anywhere from 1 s to 28 s after the request -- so an occasional first view takes
considerably longer than the figure above. The view waits it out rather than
failing, showing **Connecting** and then **Buffering** while it does, because a
camera that is seconds from streaming is worth waiting for. A camera that really
is unreachable gives up after about a minute.

Nothing wakes a battery camera unless you are watching it. A dashboard card's
thumbnail comes from the cloud, the periodic sensor refresh skips battery
cameras, and they are excluded from the start-up warm-up that mains cameras
get. They are warmed on a motion event, but only because the camera has already
woken itself to record.
...
- **A battery camera's first view after a long sleep shows nothing, then works
  ...
  sometimes acks the request and then never starts the stream; the integration
  gives up on that attempt and retries, and the second attempt streams normally,
  typically within seconds. It used to hold the dud attempt for 75 s before
  retrying and now abandons it as soon as the camera has been heard from and
  stayed silent, which roughly halves the wait. If a battery camera fails
  repeatedly rather than once, that is worth a bug report; a single dud attempt
  after a quiet period is the camera, not the integration.
