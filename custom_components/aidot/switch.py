...
#: Detection types the camera reports under getRoiHuman. `publicZone` rides in
#: the same object but is not a detection type -- it is carried through by the
#: library's read-modify-write and deliberately gets no switch.
DETECTION_TYPE_TRANSLATION_KEYS: dict[str, str] = {
    "humanDetect": "detect_human",
    "vehicleDetect": "detect_vehicle",
    "packageDetect": "detect_package",
    "petDetect": "detect_pet",
}

#: For the error text a rejected write shows, which is not an entity name.
DETECTION_TYPE_NAMES: dict[str, str] = {
    "humanDetect": "Human detection",
    "vehicleDetect": "Vehicle detection",
    "packageDetect": "Package detection",
    "petDetect": "Pet detection",
}


#: The siren's automatic trigger: the master, and the two events that fire it.
#: Named for the consequence rather than the wire key -- someone toggling these
#: is deciding whether a siren goes off in their house.
AUTO_ALARM_TRANSLATION_KEYS: dict[str, str] = {
    "autoAlarm": "auto_siren",
    "motionDetection": "auto_siren_on_motion",
    "humanDetect": "auto_siren_on_person",
}

AUTO_ALARM_NAMES: dict[str, str] = {
    "autoAlarm": "Automatic siren",
    "motionDetection": "Automatic siren on motion",
    "humanDetect": "Automatic siren on person",
}


class AidotAutoAlarmSwitch(AidotEntity, SwitchEntity):
    """The camera's automatic siren, and what sets it off.

    Separate from the siren entity, which sounds it now. This decides whether
    the camera does so by itself. The master and the triggers are separate
    entities on purpose: the library writes one flag at a time against the
    camera's own object, so changing a trigger cannot arm the master.
    """

    _attr_entity_category = EntityCategory.CONFIG

    def __init__(self, coordinator, key: str) -> None:
        super().__init__(coordinator, key=f"auto_alarm_{key}")
        self._key = key
        self._attr_translation_key = AUTO_ALARM_TRANSLATION_KEYS[key]

    @property
    def is_on(self) -> bool | None:
        """None when the camera has not told us -- unknown, not off.

        Of all the settings here this is the one least safe to guess at: a
        confident "off" on an alarm the camera never reported would read as
        "the siren will not sound" without the camera having said so.
        """
        flags = (self.coordinator.camera_extras or {}).get("alarm")
        if not isinstance(flags, dict):
            return None
        return flags.get(self._key)

    async def _async_set(self, on: bool) -> None:
        label = AUTO_ALARM_NAMES.get(self._key, self._key)
        await self.async_run_command(
            self.coordinator.device_client.async_set_auto_alarm(self._key, on),
            f"turn {'on' if on else 'off'} {label}",
        )
        await self.coordinator.async_refresh_camera_extras(only="alarm")
...
class AidotDetectionTypeSwitch(AidotEntity, SwitchEntity):
    """One detection type on one camera: human, vehicle, package or pet.

    The cameras have always typed their detections -- `humanDetect` reads 1 on
    a stock A000088 and A001064 -- while Home Assistant offered only motion
    on/off and a sensitivity number. Same shape as the sound detectors: an MQTT
    round trip the coordinator refreshes slowly, so read the cache and force a
    ...
    def __init__(self, coordinator, key: str) -> None:
        super().__init__(coordinator, key=f"detect_{key}")
        self._key = key
        self._attr_translation_key = DETECTION_TYPE_TRANSLATION_KEYS[key]

    @property
    def is_on(self) -> bool | None:
        """None when the camera has not told us -- unknown, not off.

        A battery camera answers nothing at all here (it is asleep), and four
        confidently-off switches would be a state it never reported.
        """
        flags = (self.coordinator.camera_extras or {}).get("detect")
        if not isinstance(flags, dict):
            return None
        return flags.get(self._key)

    async def _async_set(self, on: bool) -> None:
        label = DETECTION_TYPE_NAMES.get(self._key, self._key)
        await self.async_run_command(
            self.coordinator.device_client.async_set_detection_type(self._key, on),
            f"turn {'on' if on else 'off'} {label}",
        )
        await self.coordinator.async_refresh_camera_extras(only="detect")
        self.async_write_ha_state()
...
async def async_setup_entry(
    ...
    def _add_new_switches() -> None:
        ...
        # The serve-audio toggle only applies to SDES cameras.  Not just
        # battery ones: the gate below is is_sdes_camera, and the mains
        # A001064 PTZ is SDES and does get the switch.
        new += [
        ...
        # Detection types, same late-add path. Only the four we have names for:
        # the camera also reports publicZone in this object, which is not a
        # detection type and is carried through by the library instead.
        for dev_id, c in coordinator.camera_coordinators.items():
            flags = (getattr(c, "camera_extras", {}) or {}).get("detect")
            if not isinstance(flags, dict):
                continue
            for key in flags:
                if key not in DETECTION_TYPE_TRANSLATION_KEYS:
                    continue
                marker = f"{dev_id}:detect:{key}"
                if marker in registered:
                    continue
                registered.add(marker)
                new.append(AidotDetectionTypeSwitch(c, key))

        # Automatic siren, same late-add path.
        for dev_id, c in coordinator.camera_coordinators.items():
            flags = (getattr(c, "camera_extras", {}) or {}).get("alarm")
            if not isinstance(flags, dict):
                continue
            for key in flags:
                if key not in AUTO_ALARM_TRANSLATION_KEYS:
                    continue
                marker = f"{dev_id}:auto_alarm:{key}"
                if marker in registered:
                    continue
                registered.add(marker)
                new.append(AidotAutoAlarmSwitch(c, key))
