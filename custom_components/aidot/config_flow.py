...
class AidotOptionsFlow(OptionsFlow):
    ...
    async def async_step_init(
        ...
        current_pin_h264 = self.config_entry.options.get(
            CONF_SDES_PIN_H264, DEFAULT_SDES_PIN_H264
        )
        current_sdes_audio = self.config_entry.options.get(
            CONF_SDES_AUDIO, DEFAULT_SDES_AUDIO
        )
        current_sdes_audio_gain = self.config_entry.options.get(
        ...
        current_sdes_push = self.config_entry.options.get(
            CONF_SDES_PUSH, DEFAULT_SDES_PUSH
        )
        return self.async_show_form(
            step_id="init",
            data_schema=vol.Schema(
                {
                    ...
                    # SDES serve mode: publish into go2rtc (push, recommended)
                    # instead of the legacy HTTP pull serve. See const.py.
                    vol.Optional(
                        CONF_SDES_PUSH, default=current_sdes_push
                    ): bool,
                    ...
                    vol.Optional(
                        ...
                        default=self.config_entry.options.get(
                            CONF_STARTUP_PREWARM, DEFAULT_STARTUP_PREWARM),
                    ): bool,
                    vol.Optional(
