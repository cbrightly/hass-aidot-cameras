...
class AidotOptionsFlow(OptionsFlow):
    ...
    async def async_step_streaming(
        ...
        return self.async_show_form(
            ...
            data_schema=vol.Schema(
                {
                    vol.Optional(CONF_SERVE_PORT_BASE, default=current): vol.All(
                        vol.Coerce(int), vol.Range(min=1024, max=65100)
                    ),
                    vol.Optional(
                    ...
                    vol.Optional(CONF_SDES_AUDIO, default=current_sdes_audio): bool,
                    ...
                    vol.Optional(CONF_SDES_PIN_H264, default=current_pin_h264): bool,
                    ...
                    vol.Optional(
                        CONF_DIRECT_PUBLISH,
                        default=opts.get(CONF_DIRECT_PUBLISH, DEFAULT_DIRECT_PUBLISH),
                    ): bool,
                }
            ),
        )
    ...
    async def async_step_notifications(
        ...
        if user_input is not None:
            cfg.update(
                {
                    CONF_NOTIFY_TARGETS: list(
                        user_input.get(CONF_NOTIFY_TARGETS) or []
                    ),
        ...
        return self.async_show_form(
            ...
            data_schema=vol.Schema(
                {
                    vol.Optional(
                        CONF_NOTIFY_TARGETS, default=stored
                    ): self._targets_selector(stored),
                    vol.Optional(
                        CONF_NOTIFY_COOLDOWN_S,
                        default=cfg.get(
                            CONF_NOTIFY_COOLDOWN_S, DEFAULT_NOTIFY_COOLDOWN_S
                        ),
                    ): vol.All(vol.Coerce(int), vol.Range(min=0, max=3600)),
                    vol.Optional(
                        CONF_NOTIFY_TITLE,
                        default=cfg.get(CONF_NOTIFY_TITLE, DEFAULT_NOTIFY_TITLE),
                    ): str,
                    vol.Optional(
                        CONF_NOTIFY_MESSAGE,
                        default=cfg.get(CONF_NOTIFY_MESSAGE, DEFAULT_NOTIFY_MESSAGE),
                    ): str,
                }
            ),
        )
    ...
    async def async_step_camera_notifications(
        ...
        if user_input is not None:
            ...
            self._camera_name = dict(cameras).get(
                self._camera_dev_id, self._camera_dev_id
            )
            ...
            data_schema=vol.Schema(
                {
                    vol.Required("camera"): selector.SelectSelector(
                        selector.SelectSelectorConfig(
                            options=[
                                {"value": dev_id, "label": name}
                                for dev_id, name in cameras
                            ],
                            mode=selector.SelectSelectorMode.DROPDOWN,
                        )
                    ),
            ...
            data_schema=vol.Schema(
                {
                    vol.Required(
                        CONF_NOTIFY_EVENTS,
                        default=cam.get(CONF_NOTIFY_EVENTS, NOTIFY_EVENTS_OFF),
                    ): selector.SelectSelector(
                        selector.SelectSelectorConfig(
