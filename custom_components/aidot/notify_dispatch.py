...
from .proxy import NOTIFY_URL_TTL, sign_playback_url
...
def resolve_camera_config(
    options: Mapping[str, Any], dev_id: str
) -> CameraNotifyConfig | None:
    ...
    if events == NOTIFY_EVENTS_OFF:
        return None
    targets = list(cam.get(CONF_NOTIFY_TARGETS) or []) or list(
        cfg.get(CONF_NOTIFY_TARGETS) or []
    )
    try:
...
def event_kind(event: Mapping[str, Any]) -> str:
    ...
    return (
        KIND_PERSON
        if str(code if code is not None else "") == PERSON_EVENT_CODE
        else KIND_MOTION
    )
...
def build_payload(
    *,
    ...
    if uuid := event.get("eventUuid"):
        # A pushed link is tapped whenever the user gets to their phone, not
        # at browse time - the default 6 h window would 403 an overnight
        # notification. Replay is still bounded: the signing secret is
        # regenerated on every HA start, so no link outlives a restart.
        clip = sign_playback_url(dev_id, str(uuid), now=now, ttl=NOTIFY_URL_TTL)
        data["url"] = clip
        data["clickAction"] = clip
        # The companion app's first-class video key (MessagingManager.kt,
        # VIDEO_URL = "video"): it fetches the clip, pulls frames out with
        # MediaMetadataRetriever and animates them inside the notification, so
        # the clip is visible without opening anything. A RELATIVE url is
        # correct here - the app resolves it against the server URL and fetches
        # it authenticated (`requiresAuth = !isAbsoluteUrl`), and our signed
        # path is happy either way. Android takes the non-WebKit branch of the
        # proxy, so it is handed the H.264 transcode, which the retriever can
        # decode; the HEVC passthrough is WebKit-only.
        data["video"] = clip
    if camera_entity_id:
...
class AidotMotionNotifier:
    ...
    def attach(self) -> None:
        ...
        coordinators: Mapping[str, Any] = (
            getattr(manager, "camera_coordinators", None) or {}
        )
    ...
    async def _send_inner(
        self, dev_id: str, coord: Any, event: Mapping[str, Any]
    ) -> None:
        ...
        for target in cfg.targets:
            try:
                # blocking=True on purpose. With blocking=False the call returns
                # as soon as the payload validates and the platform runs in a
                # detached task, so a stale mobile_app registration or an
                # unreachable push service fails where nothing observes it -
                # and `sent` below would then record a delivery that was never
                # attempted, silently clearing the no-target warning. The whole
                # value of this feature is that the message got through, so the
                # send is awaited and a failure is logged and counted as one.
                await self.hass.services.async_call(
                    "notify", target, payload, blocking=True
                )
                sent = True
            except Exception as exc:  # unknown service, bad payload, delivery ...
                _LOGGER.warning(
