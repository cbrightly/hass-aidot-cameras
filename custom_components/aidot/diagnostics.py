...
def _package_versions(names: "tuple[str, ...]" = _REPORTED_PACKAGES) -> dict:
    ...
    for name in names:
        try:
            ...
            out[name] = None
    # The version alone does not answer "which upstream is this". Upstream
    # shipped the same private-API shape under 0.3.53 and 0.3.56 with a
    # five-day excursion to the other one in between, so the library detects
    # the shape by capability at import and publishes the answer. Report that
    # too -- it is the axis that actually distinguishes two installs.
    try:
        from aidot_cameras._upstream import UPSTREAM_SHAPE

        out["upstream_shape"] = UPSTREAM_SHAPE
    except Exception:  # pragma: no cover - diagnostics must not raise
        out["upstream_shape"] = None
    return out


def _media_path(device_client: Any) -> Any:
    """ "direct" or "relay" for the live session's media, or None.

    The receipt for the connection-mode option: the option states a
    preference, the camera decides where to send from, and this is what
    actually happened (SdesSession.media_stats()["media_path"], stamped by
    the bridge from the first packet's source address). A label, not an
    address, so it needs no redaction. Never raises - diagnostics must not.
    """
    try:
        ...
        if not callable(stats_fn):
            return None
        return stats_fn().get("media_path")
    except Exception:  # pragma: no cover - diagnostics must not raise
        return None
...
async def async_get_config_entry_diagnostics(
        ...
        cameras.append(
            {
                ...
                "media_path": _media_path(dc.device_client),
            }
        )
    ...
    return {
        ...
        # importlib.metadata lists site-packages and reads METADATA from disk,
        # which Home Assistant flags as a blocking call on the event loop.
        "versions": await hass.async_add_executor_job(_package_versions),
        "lights": lights,
        "cameras": cameras,
    }
