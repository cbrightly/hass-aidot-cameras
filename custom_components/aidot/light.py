    ...
    def _add_new_lights() -> None:
        ...
        if new_flood:
            ...
            for c in new_flood.values():
                ...
                remove_stale_switch_entity(
                    hass, c.device_client.info.dev_id, "floodlight"
                )
...
class AidotLight(CoordinatorEntity[AidotDeviceUpdateCoordinator], LightEntity):
    ...
    def __init__(self, coordinator: AidotDeviceUpdateCoordinator) -> None:
        ...
        self._attr_max_color_temp_kelvin = (
            getattr(coordinator.device_client.info, "cct_max", None)
            or DEFAULT_MAX_KELVIN
        )
        self._attr_min_color_temp_kelvin = (
            getattr(coordinator.device_client.info, "cct_min", None)
            or DEFAULT_MIN_KELVIN
        )

        entry = getattr(coordinator, "config_entry", None)
        self._attr_device_info = aidot_device_info(
            coordinator.device_client.info,
        ...
        if coordinator.device_client.info.enable_rgbw:
            # Dual-mode (RGBW + color temp) bulbs return both registers in the
            # ambiguous login-sync, so the active mode is unknown at startup.
            # Default to COLOR_TEMP - the common resting state - rather than
            # surfacing a stale RGB color; _update_status flips to RGBW as soon
            # as the device sends an unambiguous color delta.
    ...
    def _update_status(self) -> None:
        if self.coordinator.data is None:
            return
        # Without a LAN session the library's status holds only defaults
        # (on=False and no colour), which are not the device's real state.  A
        # bulb that is powered on but off-LAN must read as unknown rather than
        # be reported "off" - control is LAN-only, so a fabricated state would
        # also invite a command that cannot be delivered.
        if not getattr(self.coordinator, "has_lan_state", True):
        ...
        # Follow the device's active color mode when it is known. Dual-mode bulbs
        # send deltas carrying only the active register (CCT-only or RGBW-only),
        # which the library records as active_color_mode; the ambiguous login-sync
        # (both registers) leaves it None, so we keep the current mode rather than
        # surfacing the stale RGBW color.
        if ColorMode.RGBW in self._attr_supported_color_modes:
            active = getattr(self.coordinator.data, "active_color_mode", None)
            if active == "cct":
                self._attr_color_mode = ColorMode.COLOR_TEMP
            elif active == "rgbw":
    ...
    def _lan_required(self, exc: Exception) -> HomeAssistantError:
        """Translate the library's LAN-offline refusal into a clear HA error.

        Control of these devices is LAN-only: upstream raises a bare
        ``ConnectionError("Device offline")`` when there is no authenticated
        session, which would otherwise surface to the user as HTTP 500.
        """
        return HomeAssistantError(
            f"{self.name} cannot be controlled right now: it is not reachable "
            "over the local network. These devices are controlled on the LAN, "
            "so Home Assistant must be able to reach the device directly "
    ...
    async def async_turn_on(self, **kwargs: Any) -> None:
        try:
            ...
            raise self._lan_required(exc) from exc
    ...
    async def _async_turn_on(self, **kwargs: Any) -> None:
        if ATTR_BRIGHTNESS in kwargs:
            ...
            # Optimistically record the active mode too, so a subsequent ambiguous
            # sync (e.g. the polled getDevAttr keepalive) does not revert this
            # command before the device echoes an unambiguous delta.
            self.coordinator.data.active_color_mode = "cct"
            ...
            self.coordinator.data.active_color_mode = "rgbw"
