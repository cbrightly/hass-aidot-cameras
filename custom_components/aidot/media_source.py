...
class AidotMediaSource(MediaSource):
    ...
    async def async_resolve_media(self, item: MediaSourceItem) -> PlayMedia:
        ...
        # Only a cloud clip is playable, and its identifier is the one form that
        # uses a slash: device_id/event_uuid. Every other identifier the browse
        # tree emits uses "|" and is a folder or an informational leaf.
        #
        # Home Assistant's media browser resolves ANY non-expandable child when
        # it is tapped (can_play only governs whether the play button is drawn,
        # not whether the tap resolves), so those "|"-identifiers reach here.
        # They must explain themselves, not raise a developer-facing error.
        if "/" not in identifier:
            if f"|{SOURCE_SD}|" in identifier and not identifier.endswith("|none"):
                raise Unresolvable(
                    "This recording is on the camera's SD card. Home Assistant "
                    "lists on-device recordings for reference but cannot play "
                    "them - open the clip in the AiDot app to watch it."
                )
            if "|" in identifier:
                raise Unresolvable("This is a listing, not a playable recording.")
            raise Unresolvable(f"Invalid identifier: {item.identifier!r}")

        device_id, event_uuid = identifier.split("/", 1)
    ...
    async def _day_empty_message(self, dc, total: int | None, fetched: int) -> str:
        ...
        if total:
            return (
                f"Could not load this day's recordings - the server "
    ...
    async def _build_days(self, device_id: str) -> BrowseMediaSource:
        ...
        windows = day_windows(now_ms, CLOUD_LOOKBACK_DAYS, dt_util.DEFAULT_TIME_ZONE)
        ...
        if summaries:
            children = [
                BrowseMediaSource(
                    ...
                    title=(f"{label}  ({count})" if count is not None else f"{label}"),
                    can_play=False,
                    can_expand=True,
                )
    ...
    async def _build_day(self, device_id: str, day: str) -> BrowseMediaSource:
        ...
        for ev in events:
            ...
            if ts_ms:
                local = dt_util.as_local(datetime.fromtimestamp(ts_ms / 1000, tz=UTC))
        ...
        # clips.
        if warm_ids:
            self.hass.async_create_task(async_prewarm_events(device_id, warm_ids[:10]))
        ...
        if not children:
            ...
            children = [
                BrowseMediaSource(
                    ...
                    media_class=MediaClass.DIRECTORY,
                    media_content_type="",
                    title=await self._day_empty_message(dc, total, len(events)),
                    can_play=False,
                    can_expand=False,
                )
            ]
    ...
    async def _build_sd_days(self, device_id: str) -> BrowseMediaSource:
        ...
        if cache is not None:
            ...
            if children and not cache.complete:
                ...
                children.append(
                    self._sd_note(device_id, sd_empty_message(cache, card_present))
                )

        if not children:
            children = [self._sd_note(device_id, sd_empty_message(cache, card_present))]
        ...
        if records:
            for index, record in enumerate(records):
                ...
                children.append(
                    BrowseMediaSource(
                        domain=DOMAIN,
                        # Not playable - stage 3 (pulling a clip off the card)
                        # was closed as not feasible. can_play=False keeps the
                        # play button off, but the media browser still resolves
                        # this leaf when a user taps the row, so the identifier
                        # IS resolved: async_resolve_media routes it to the
                        # "open it in the AiDot app" explanation.
