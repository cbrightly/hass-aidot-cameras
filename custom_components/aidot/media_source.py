...
from .sd_recordings import map_days, record_days, sd_empty_message
...
class AidotMediaSource(MediaSource):
    ...
    def _build_sources(self, device_id: str) -> BrowseMediaSource:
        ...
        if cache is None or not cache.answered:
        ...
        if cache is not None:
            ...
            if children and not cache.complete:
                ...
                children.append(self._sd_note(
                    device_id, sd_empty_message(cache)))

        if not children:
            children = [self._sd_note(device_id, sd_empty_message(cache))]
