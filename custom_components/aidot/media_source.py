...
from .proxy import PROXY_PATH, async_prewarm_events
...
class AidotMediaSource(MediaSource):
    ...
    async def async_resolve_media(self, item: MediaSourceItem) -> PlayMedia:
        ...
        # off this unauthenticated endpoint avoids an SSRF/LFI vector.
        q = urllib.parse.urlencode({"device": device_id, "event": event_uuid})
        return PlayMedia(f"{PROXY_PATH}?{q}", "video/mp4")
