...
# nothing; only a genuinely cold open waits, and it waits with a spinner instead
# of failing. On the HLS path HA's own 10s timeout cancels us first, which is no
# worse than today - that path already gets a 404 from the same cold stream.
_PUSH_PUBLISHER_WAIT_S = 30.0
            ...
            if getattr(dc, "is_sdes_camera", False):
                ...
                if _push:
                    ...
                    attached = pub_state == "live" or await self._await_publisher_attached(
                        _PUSH_PUBLISHER_WAIT_S
                    )
                    if not attached:
                        # go2rtc never showed a publisher - either the open is
                        # slower than the budget or go2rtc cannot be read. Fall
                        # back to the library's own readiness signal and hand HA
                        # the URL regardless: a URL that may 404 still beats
                        # None, which makes HA's provider tear down every
