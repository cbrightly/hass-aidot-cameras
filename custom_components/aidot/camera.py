...
class AidotCamera(CoordinatorEntity[AidotCameraUpdateCoordinator], Camera):
    ...
    async def _publisher_is_attached(self, push: bool) -> bool:
        ...
        if not streams:
            return True  # go2rtc unreachable - do not thrash the session
        entry = streams.get(self._go2rtc_name)
        if not isinstance(entry, dict):
            return False  # stream definition is gone; a restart must re-register
        producers = entry.get("producers") or []
        return any(not p.get("url") for p in producers if isinstance(p, dict))
    ...
    async def stream_source(self) -> str | None:
        ...
        if not self._setup_complete:
            ...
            # go2rtc supports.
            #
            # Except in SDES push mode, where nothing ever binds the local serve
            # (the media is published into go2rtc instead), so there is no source
            # to offer and None correctly means "still image, retry".
        ...
        try:
            if dc.stream_rtsp_url is None or not await self._publisher_is_attached(_push):
                try:
