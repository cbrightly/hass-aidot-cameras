...
class AidotCamera(CoordinatorEntity[AidotCameraUpdateCoordinator], Camera):
    ...
    def _connect_options(self) -> tuple[bool, bool, bool, bool]:
        """Return (fast_connect, sdes_audio, sdes_fast_liveplay, sdes_adaptive)."""
        opts = self.coordinator.config_entry.options if self.coordinator.config_entry else {}
        return (
            bool(opts.get(CONF_FAST_CONNECT, DEFAULT_FAST_CONNECT)),
            bool(opts.get(CONF_SDES_AUDIO, DEFAULT_SDES_AUDIO)),
            bool(opts.get(CONF_SDES_FAST_LIVEPLAY, DEFAULT_SDES_FAST_LIVEPLAY)),
            bool(opts.get(CONF_SDES_ADAPTIVE, DEFAULT_SDES_ADAPTIVE)),
        )
    ...
    async def _prewarm_stream(self) -> None:
        ...
        dc = self.coordinator.device_client
        fast_connect, sdes_audio, sdes_fast_liveplay, sdes_adaptive = self._connect_options()
        try:
            await dc.start_keepalive(
                rtsp_push_url=self._serve_url,
                fast_connect=fast_connect,
                sdes_audio=sdes_audio,
                sdes_fast_liveplay=sdes_fast_liveplay,
                sdes_adaptive=sdes_adaptive,
    ...
    async def _unpublish_from_go2rtc(self) -> None:
        ...
        except Exception:  # noqa: BLE001 - best-effort teardown hygiene
            pass
        ...
        try:
            if dc.stream_rtsp_url is None:
                try:
                    ...
                    _LOGGER.info(
                        "Started HTTP stream serve for %s → %s (fast_connect=%s)",
                        self._rtsp_name, serve_url, fast_connect,
                    )
                except Exception as exc:
                    _LOGGER.warning(
