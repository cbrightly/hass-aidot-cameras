...
#: How long a go2rtc reachability answer is reused across cameras.
_GO2RTC_REACHABLE_TTL_S = 60.0


def _direct_publish_enabled() -> bool:
    """Whether the library publishes in-process (the direct publish option).

    False with a library that predates direct publish, so a DTLS camera is never
    switched to push mode against a library that would serve it through ffmpeg.
    """
    try:
        from aidot_cameras.camera.rtsp_publish import direct_publish_enabled
    except ImportError:
        return False
    return direct_publish_enabled()
...
#
# Sized against a cold SDES open, historically measured at 16-22s (MQTT
# signalling + ICE + SDES + ffmpeg).  That figure predates the cold-start work:
# re-measured 2026-09-07 over 11 real cold opens, the serve binds in 2.6-3.6s.
# The budget is deliberately NOT retuned to match - it is an upper bound on a
# wait that costs nothing when the publisher arrives early, and the failure it
# prevents is expensive.  Below it, the first view is *guaranteed* to fail: go2rtc answers a
...
#
# **The "one bad first click" this used to describe is NO LONGER THE TRADE.**
# That was written when one budget served every caller. `_publisher_wait_budget`
# now splits them: this short one is the HLS path only, and the view path - the
# one a card tap uses, which reaches stream_source() through the go2rtc provider
# and has no timeout at all - gets `_VIEW_PUBLISHER_WAIT_BATTERY_S` (75 s).
# Measured 2026-09-12 on two charged A001513s: the view path answered first try
# in 0.3 s and 6.2 s, no retries.
#
# What remains true is the HLS limit: a cold battery camera cannot beat HA's
# 10 s clock, so `camera.record` and HLS on a cold battery camera still fail and
# retry. Do not read that as the live view failing - it is a different path with
# a budget seven times larger, and testing the first click with `camera.record`
# measures this comment rather than the card.
_PUSH_PUBLISHER_WAIT_S = float(os.environ.get("AIDOT_PUSH_PUBLISHER_WAIT_S", "8.5"))
# How often to ask go2rtc whether the publisher has attached yet.
#
# This is pure latency on every cold open: the wait ends on a poll, so a 1.0s
# interval spends on average half a second, and up to a full one, after the
# publisher is already there. Against a measured cold battery open that reaches
# the card in ~13s, and a warm one that reaches it in ~1s, that is worth
# tightening. The poll is a localhost GET of go2rtc's stream list and only runs
# while a cold open is in flight, so four a second is cheap.
_PUSH_PUBLISHER_POLL_S = 0.25

# The source a PUSH camera's go2rtc stream definition is created with.
#
# go2rtc will not hold a stream with no source, and it refuses a publish into a
# name it does not know, so the definition has to exist with SOMETHING before
# the ANNOUNCE. It used to be the camera's own local serve URL, which is the
# port that push mode specifically never binds - the media arrives by ANNOUNCE
# instead. That left every published camera's definition naming a local address
# we had stopped listening on:
#
#   * whenever no publisher is attached, go2rtc dials it and the viewer gets
#     "connection refused" rather than an empty stream, and
#   * anything else that later binds that port is served to viewers AS this
#     camera. Under A4 (push by default) those serve ports are never ours, so
#     that is a live address on the box pointing at a stranger.
#
# Port 1 cannot be bound without root, and refuses instantly rather than
# hanging, so this is inert by construction rather than by luck.
_PUSH_PLACEHOLDER_SRC = "rtsp://127.0.0.1:1/aidot-placeholder"
...
# The budgets above exist to fit HA's 10s CAMERA_STREAM_SOURCE_TIMEOUT, and
# that timeout wraps exactly one caller: `Camera.async_create_stream`, the HLS
# path. Every other caller reaches stream_source() with no clock at all - HA's
# go2rtc WebRTC provider (`_update_stream_source`) and third-party cards
# (`async_get_stream_source`), both verified against HA 2026.8.3 - and neither
# retries a URL that fails, because go2rtc answers a consumer on a stream whose
# publisher has not landed with an immediate error. Measured on the box
# 2026-09-03: a battery A001513's cold open landed its publisher at +11.1s, the
# 8.5s budget had handed the URL over at +7.2s, and the viewer got
#     dial tcp 127.0.0.1:18812: connect: connection refused
# and stopped. The next cold open took +5.3s and worked - the short budget
# turns the first click into a coin toss on exactly the cameras that are always
# cold (battery, excluded from prewarm).
#
# So the budget is sized by who is asking. async_create_stream marks its own
# call and keeps the short budgets; everyone else waits long enough for a cold
# open. A warm camera pays nothing either way: the wait ends the instant the
# publisher, or the local serve, is ready.
#
# Sized against a cold BATTERY open, which is the slow one and the one the 8.5s
# budget was cutting off. Measured 2026-09-03 with a 150s settle so each open
# was genuinely cold: the publisher attached at +5.5s and +10.4s, and an earlier
# one at +11.1s. The 8.5s budget sits in the middle of that spread, which is
# why the first click failed about half the time while the vendor app - which
# has no such cutoff - pulls the same camera up almost instantly.
#
# 45s is sized for the worse case rather than the common one: a cold open that
# stalls is abandoned by the library at about +25s and its retry serves media by
# roughly +35-40s, so a budget that stops short of that turns a recoverable
# first click back into a failed one. The cost is paid only by a camera that is
# genuinely unreachable, which now spins for 45s before erroring instead of
# 8.5s - a worse wait for a broken camera, in exchange for a working first
# click on a healthy one.
_VIEW_PUBLISHER_WAIT_S = float(os.environ.get("AIDOT_VIEW_PUBLISHER_WAIT_S", "20"))

# A BATTERY camera's view budget, which has to be much larger, and the reason is
# the camera rather than us.
#
# A cold battery open is mostly the camera waking, and that wake is wide:
# measured on one camera in one day, its own wake announcement arrived +0.9 s,
# +5.4 s, +5.9 s, +10.9 s and +27.9 s after the open. Most of those reach the
# card in 8-13 s. The slow tail does not: on the +27.9 s open the first attempt
# was abandoned to the retry and media arrived at +46.7 s, which a 45 s budget
# missed by 1.7 s - so the click failed even though the camera was seconds from
# streaming.
#
# There is nothing to shorten there. The library's own signalling is under a
# second of it, and holding the offer back to wait for the camera was measured
# and makes it worse. So the choice is between a first click that sometimes
# fails and one that sometimes takes a while, and for a camera that is about to
# work, waiting is the better answer: the card shows Connecting / Buffering
# throughout, so it reads as slow rather than broken.
#
# 75 s clears the measured tail with room. It is only ever spent by a battery
# camera that is genuinely still waking - a warm one returns in about a second,
# and every other camera type keeps the short budget above. PARALLEL_UPDATES is
# 0, so a camera sitting in this wait blocks nothing else.
_VIEW_PUBLISHER_WAIT_BATTERY_S = float(
    os.environ.get("AIDOT_VIEW_PUBLISHER_WAIT_BATTERY_S", "75")
)


def _view_publisher_budget(battery: bool) -> float:
    """The non-HLS view budget for this kind of camera."""
    return _VIEW_PUBLISHER_WAIT_BATTERY_S if battery else _VIEW_PUBLISHER_WAIT_S


_VIEW_SERVE_READY_WAIT_S = float(os.environ.get("AIDOT_VIEW_SERVE_READY_WAIT_S", "25"))
_HLS_SERVE_READY_WAIT_S = 7.0

# True only while HA's own async_create_stream is calling stream_source().
_STREAM_SOURCE_HLS: ContextVar[bool] = ContextVar(
    "aidot_stream_source_hls", default=False
)
...
class AidotCamera(CoordinatorEntity[AidotCameraUpdateCoordinator], Camera):
    ...
    def __init__(self, coordinator: AidotCameraUpdateCoordinator) -> None:
        ...
        self._cached_image_ts: float = (
            0.0  # monotonic time of last successful fetch/check
        )
    ...
    async def async_added_to_hass(self) -> None:
        ...
        _LOGGER.debug(
            ...
            self._rtsp_name,
            int(_STALE_STREAM_CHECK.total_seconds()),
        )
    ...
    async def _evict_stale_stream(self, _now: datetime.datetime | None = None) -> None:
        ...
        try:
            ...
            _LOGGER.debug("stale-stream stop failed for %s: %s", self._rtsp_name, exc)
    ...
    def _connect_options(self) -> dict[str, Any]:
        ...
        return {
            ...
            "sdes_audio": _audio,
            "sdes_audio_gain_db": float(
                opts.get(CONF_SDES_AUDIO_GAIN_DB, DEFAULT_SDES_AUDIO_GAIN_DB)
            ),
            "sdes_fast_liveplay": bool(
                opts.get(CONF_SDES_FAST_LIVEPLAY, DEFAULT_SDES_FAST_LIVEPLAY)
            ),
        }
    ...
    def _stream_idle_s(self) -> float | None:
        ...
        release.  A mains camera has no battery to protect, and each warm one
        holds a concurrent-stream slot and keeps decrypting, which is why the
        library's stream cap is sized to the fleet.

        This used to say "a cold wake on these cameras measures 16-22s, which is
        long enough that a live view reads as broken".  RE-MEASURED 2026-09-07
        across 11 real cold opens on the live fleet: the serve bound in 2.6-3.6s,
        median 2.9s, SDES and DTLS alike.  The cold-start work closed that gap.
        See the note on DEFAULT_MAINS_IDLE_S in const.py -- and note that a
        camera which stops sending when unwatched cannot be held warm at all,
        so a 0 window leaves the watchdog restarting it indefinitely.
    ...
    async def _prewarm_stream(self) -> None:
        ...
        if not getattr(getattr(dc, "status", None), "online", True):
            _LOGGER.debug("Skipping prewarm for %s: camera is offline", self._rtsp_name)
            return
        _push = await self._push_mode()
        # go2rtc rejects an RTSP publish to a name it does not know, and it will
        # not hold a source-less stream, so the definition (with its inert http
        # placeholder) has to exist before the publish - the same order the view
        # path uses. Setup used to do this for every camera, which left an
        # unviewed push camera holding a producer go2rtc could only fail to dial;
        # doing it here instead keeps the placeholder tied to a publish that is
        # actually on its way.
        #
        # Only with no session of our own: `PUT /api/streams` replaces the
        # definition wholesale and would drop a live publisher (see the view
        # path). A set stream_rtsp_url means a publisher was established, and a
        # publisher that has since died is handled on the view path's tri-state,
        # so skipping the PUT here costs nothing.
        try:
            if _push and getattr(dc, "stream_rtsp_url", None) is None:
                # Registration can fail without raising -- None when go2rtc is
                # disabled, or when the PUT times out because it is restarting
                # or reachable only over a unix socket. Pushing anyway sends
                # RTSP at a stream name go2rtc does not know, so the publish is
                # rejected and the ffmpeg plus the keepalive retry loop churn
                # against a camera nobody is even watching. Skip the prewarm
                # instead; the view path re-registers when someone does look.
                if await self._publish_to_go2rtc(self._serve_url) is None:
                    _LOGGER.debug(
                        "Motion prewarm skipped for %s: go2rtc did not accept "
                        "the stream definition, so there is nothing to push to",
                        self._rtsp_name,
                    )
                    return
            await dc.start_keepalive(
                rtsp_push_url=self._push_serve_url if _push else self._serve_url,
            ...
            self.hass.async_create_task(self.coordinator.async_piggyback_sd_refresh())
        except Exception as exc:
    ...
    async def _prefetch_thumbnail(self) -> None:
        ...
        try:
            ...
            _LOGGER.debug(
                "Thumbnail bytes fetch failed for %s: %s", self.unique_id, exc
            )
    ...
    def _sdes_push_enabled(self) -> bool:
        ...
        that stall SDES cold starts under HA. Requires go2rtc; DTLS cameras
        keep the proven pull serve.

        The stored ``sdes_push`` option is deliberately NOT consulted. 2.19.0
        removed the toggle from the options page because publishing into go2rtc
        is the mode that works and the pull serve it could switch to is the one
        that jams - but it kept reading the key, so an entry that had turned it
        off in 2.18.x was pinned to the jamming path with no UI left to undo it.
        Honouring a setting a user can no longer see or change is worse than
        ignoring it, so this returns the working mode for every SDES camera.
        """
        dc = self.coordinator.device_client
        ...
        if getattr(dc, "is_sdes_camera", False):
            return True
        # DTLS cameras publish too once the library publishes in-process
        # (direct publish): there is then no ffmpeg or serve port to pull from.
        return _direct_publish_enabled()

    async def _push_mode(self) -> bool:
        """``_sdes_push_enabled`` with the DTLS case checked against reality.

        A DTLS camera only moves to push mode when this integration can
        actually reach go2rtc. It cannot always: Home Assistant's bundled
        go2rtc keeps its API on a unix socket and its RTSP on 18554, while
        this integration talks to 127.0.0.1:1984/8554. Home Assistant CAN
        reach its own go2rtc, which is why the pull serve works there - it
        hands HA an http:// URL and HA registers it. Handing back a push URL
        instead points HA at a port nothing listens on and the camera goes
        dark, which is a regression against the pull serve that worked.

        SDES cameras are unaffected: they have no working pull path (see
        _sdes_push_enabled), so push is their only mode either way.
        """
        if not self._sdes_push_enabled():
            return False
        if getattr(self.coordinator.device_client, "is_sdes_camera", False):
            return True
        return await self._go2rtc_reachable()

    async def _go2rtc_reachable(self) -> bool:
        """Does OUR go2rtc answer? Cached briefly and shared by all cameras.

        One probe per window rather than per camera per view: this runs on the
        setup path and on every view, and a down go2rtc must not cost each
        camera its own timeout.
        """
        store = self.hass.data.setdefault(DOMAIN, {})
        cached = store.get("go2rtc-reachable")
        now = time.monotonic()
        if cached and now - cached[0] < _GO2RTC_REACHABLE_TTL_S:
            return cached[1]
        client = Go2rtcClient(
            async_get_clientsession(self.hass), _GO2RTC_API, timeout=3.0
        )
        ok = await client.available()
        if not ok and (not cached or cached[1]):
            _LOGGER.info(
                "go2rtc did not answer at %s; cameras that can be pulled will be"
                " served that way instead of publishing",
                _GO2RTC_API,
            )
        store["go2rtc-reachable"] = (now, ok)
        return ok
    ...
    async def _publish_to_go2rtc(self, serve_url: str) -> str | None:
        ...
        if _sprop_is_unstable(self.coordinator.device_client.device_id):
            ...
            _LOGGER.debug(
                ...
                "go2rtc stream so the decoder config is rebuilt",
                self._rtsp_name,
            )
        # No AAC transcoding source. This used to add `ffmpeg:{name}#audio=aac`
        # so HA's HLS player had audio it can carry (its stream component keeps
        # aac/mp3 and drops G.711), but that transcode is where the audio
        # timestamps go wrong: the go2rtc the cameras are served by stamps it on
        # a 90 kHz clock while advertising MPEG4-GENERIC/8000, so every HLS
        # viewer plays roughly eleven times too slow.
        #
        # Measured on one camera, same stream, same 15 s wall-clock window:
        #     ?audio=pcma  ->  15.000 s container  (1.000x, exact)
        #     ?audio=aac   -> 163.143 s container  (10.88x)
        # and it is not camera-specific - 5 of 5 cameras across 3 models sat
        # between 10.76x and 11.12x.
        #
        # A newer go2rtc downstream does not rescue it. When the source is
        # offered, Home Assistant's own bundled go2rtc prefers it (go2rtc serves
        # a consumer from the first source whose codecs match) and relays the
        # bad timestamps unchanged - 166.3 s for that same window.
        #
        # So the source is simply not offered. WebRTC viewers keep audio as
        # G.711 passthrough; HA's HLS dialog loses audio, which is the trade we
        # want - a silent stream at 1x beats an 11x one with sound.
        extra: tuple[str, ...] = ()
        # A push camera never binds `serve_url`, so naming it here would point
        # the definition at a port we have stopped listening on. See
        # _PUSH_PLACEHOLDER_SRC. A pulled camera keeps its real serve URL -
        # that one go2rtc genuinely has to dial.
        src = _PUSH_PLACEHOLDER_SRC if await self._push_mode() else serve_url
        if not await client.ensure_stream(name, src, extra_sources=extra):
            # A rejected register call does not mean go2rtc cannot serve this
            # camera. A duplicate key in go2rtc's own config makes it answer
            # every PUT /api/streams with 400 while the streams stay
            # registered, so falling back on the PUT alone drops every camera
            # onto the slower HLS path at once - with nothing logged as an
            # error, the only symptom is video that looks slower than real
            # time. Ask whether the stream is there before giving it up.
            if not await client.has_stream(name):
                return None
            _LOGGER.info(
                "go2rtc rejected the register call for %s but the stream is "
                "present - using go2rtc instead of falling back",
                self._rtsp_name,
            )
    ...
    async def async_create_stream(self):
        """HA's HLS entry point - the one caller under its 10s clock.

        Marks the stream_source() it makes so the waits inside fit that clock;
        every other caller gets the cold-open-sized budgets (see
        _VIEW_PUBLISHER_WAIT_S).
        """
        token = _STREAM_SOURCE_HLS.set(True)
        try:
            return await super().async_create_stream()
        finally:
            _STREAM_SOURCE_HLS.reset(token)

    def _publisher_wait_budget(self) -> float:
        """How long a push-mode view may wait for the go2rtc publisher.

        The HLS path keeps the short budget it needs to fit Home Assistant's
        10 s clock; everything else is sized by what the camera actually needs,
        which for a battery camera is dominated by its own wake.
        """
        if _STREAM_SOURCE_HLS.get():
            return _PUSH_PUBLISHER_WAIT_S
        return _view_publisher_budget(
            bool(getattr(self.coordinator.device_client, "is_battery_camera", False))
        )

    @staticmethod
    def _serve_ready_wait_budget() -> float:
        """How long a pull-mode view may wait for the local serve to be ready."""
        return (
            _HLS_SERVE_READY_WAIT_S
            if _STREAM_SOURCE_HLS.get()
            else _VIEW_SERVE_READY_WAIT_S
        )
    ...
    async def stream_source(self) -> str | None:
        ...
        # So answer from a URL alone and let setup complete immediately; HA
        # re-calls stream_source on the first actual live-view request, at which
        # point we start normally.
        if not self._setup_complete:
            ...
            #
            # ...but a push camera must NOT register here. Nothing ever binds its
            # local http serve - the media is published into go2rtc over RTSP
            # instead - so registering that serve creates a stream whose only
            # producer is a port nothing listens on, and it stays that way for
            # every camera that is never viewed. Measured on the box 2026-08-29:
            # all three SDES cameras held such a producer, and a viewer's offer
            # died on it with
            #     dial tcp 127.0.0.1:189xx: connect: connection refused
            # while the cards sat on "Loading..".
            #
            # No registration is needed to answer this call: the push URL is
            # already an RTSP URL, so HA's provider still enables WebRTC. The
            # stream definition is created where go2rtc actually needs it -
            # immediately before a publish, by the view path below and by
            # _prewarm_stream - so the placeholder only ever exists alongside a
            # publisher that is on its way.
            #
            # Registering the push URL itself is not the alternative: it is
            # go2rtc's own address for this same stream, which makes go2rtc its
            # own producer (the zero-byte-frame self-reference loop). A
            # source-less definition is not available either - go2rtc answers
            # PUT with no src 200 but does not create the stream.
            if await self._push_mode():
                return self._push_serve_url
            ...
            # go2rtc supports.
            #
            # Only pull-mode cameras reach here, and for them the local serve is
            # the real source. A push camera returned its go2rtc RTSP URL above
            # without ever asking go2rtc, so an unreachable go2rtc cannot land it
            # on the local serve URL (which nothing would bind) and cannot make
            # this method answer None - which matters, because HA's provider
        ...
        try:
            ...
            if getattr(dc, "is_sdes_camera", False) or _push:
                # (A DTLS camera arrives here only in push mode - direct publish -
                # where it waits on its publisher exactly like an SDES camera.)
                ...
                if _push:
                    ...
                    _budget = self._publisher_wait_budget()
                    _deadline = time.monotonic() + _budget
                    attached = (
                        pub_state == "live"
                        or await self._await_publisher_attached(
                            max(0.5, _budget - _SERVE_READY_RESERVE_S)
                        )
                    )
                    if not attached:
                    ...
                    await self._await_serve_listening(
                        _serve_port(self._rtsp_name), timeout=7.0
                    )
                connected = True
                return publish_url
            ...
            if waiter is not None:
                ...
                try:
                    ...
                    # wait and we hand HA the URL anyway. The wait is short only
                    # on the HLS path (under the 10s cutoff); a WebRTC or card
                    # view gets a cold open's worth, since go2rtc dials the serve
                    # the instant the URL lands and does not retry a refusal.
                    await waiter(timeout=self._serve_ready_wait_budget())
                except Exception:
                    pass
            ...
            if connected or cancelled:
                ...
                self._set_stream_status("Camera unavailable - retrying...", error=True)
