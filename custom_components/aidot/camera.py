...
class AidotCamera(CoordinatorEntity[AidotCameraUpdateCoordinator], Camera):
    ...
    # NOTE: do not pass go2rtc_url to start_keepalive. It switches on the
    # library's own _register_with_go2rtc(), which PUTs the same stream this
    # integration already registers via _publish_to_go2rtc - a duplicate
    # registration that re-points the source while a stream is mid-flight. On this
    # fleet that left the DTLS cameras producing nothing at all, while the version
    # without it served all four. Its only purpose was to let the library ask
    # go2rtc who is watching, which no longer matters: mains cameras never release
    # (mains_idle_s defaults to 0) and battery cameras fall back to the socket
    # check. If it is ever needed again, make the library skip registration when
    # the consumer already registers.
        ...
        try:
            ...
            if _push:
                if registered is None:
                    ...
                    # pull fallback has no listener in push mode - soft-fail so
                    # HA shows the still image and retries.
                    return None
            ...
            if getattr(dc, "is_sdes_camera", False):
                ...
                if _push:
                    # Push mode: there is no local -listen port. Wait briefly on
                    # the library's serve-ready (publisher ffmpeg up); cold opens
                    # fall through and HA's worker retries against go2rtc until
                    # the publish lands - no listen slot to jam.
