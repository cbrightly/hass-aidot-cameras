...
class AidotCamera(CoordinatorEntity[AidotCameraUpdateCoordinator], Camera):
    ...
    async def _publish_to_go2rtc(self, serve_url: str) -> str | None:
        ...
        # With direct publish the live audio is the camera's own G.711, which
        # Home Assistant's HLS player cannot carry (its stream component keeps
        # aac/mp3 and drops the rest), so the Picture-card dialog would play
        # video with no sound. A transcoding source listed AFTER the live one
        # covers exactly that consumer: go2rtc serves each consumer from the
        # first source whose codecs match, starts this one on demand and stops
        # it with its last consumer, so a WebRTC viewer still gets the
        # untranscoded passthrough. Only with direct publish on: the ffmpeg
        # serve already produces AAC.
        extra = (f"ffmpeg:{name}#audio=aac",) if _direct_publish_enabled() else ()
