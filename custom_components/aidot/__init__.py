...
async def async_setup_entry(hass: HomeAssistant, entry: AidotConfigEntry) -> bool:
    ...
    # runs here - a Raspberry Pi advertises h264_cuvid with no Nvidia hardware
    # present - so each candidate is proven by decoding a sample, which costs
    # about ten seconds per codec on a first run. That is far too long to block
    # setup, and it only ever happens once per host per ffmpeg build, so it runs
    # on a daemon thread: the answer is normally cached well before the first
    # stream, and anything asking sooner just lets ffmpeg choose as before.
    # Idempotent, so a second config entry does not start a second probe.
    try:
        from aidot_cameras.camera.hwaccel import warm_decoder_cache

        warm_decoder_cache()
    except Exception:  # pragma: no cover - never block setup on a probe
