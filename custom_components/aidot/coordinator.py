...
class AidotCameraUpdateCoordinator(AidotDeviceUpdateCoordinator):
    ...
    def __init__(
        ...
        _LOGGER.debug("extras: rotation armed for %s (%s)",
                      getattr(device_client, "device_id", "?"),
                      "battery" if getattr(device_client, "is_battery_camera", False)
                      else "mains")
    ...
    async def async_list_sd_recordings(
        ...
        async with self._sd_lock:
            ...
            streak = 0 if result.answered else (
                (self.sd_cache.unanswered_streak if self.sd_cache else 0) + 1)
    ...
    def _schedule_camera_extras(self) -> None:
        ...
        try:
            ...
            _LOGGER.debug("scheduling camera extras failed for %s",
    ...
    def _schedule_camera_extras_inner(self) -> None:
        ...
        if now < getattr(self, "_extras_due", 0.0):
            _LOGGER.debug("extras[%s]: skipped, not due for %.0fs",
                          dev, getattr(self, "_extras_due", 0.0) - now)
            return
        ...
        covered = ((getattr(self, "_extras_tried", None) or set())
                   | set(getattr(self, "camera_extras", None) or {}))
        due_in = (EXTRAS_FILL_SECONDS if len(covered) < len(self._EXTRAS_KEYS)
        ...
        try:
            task = hass.async_create_background_task(
                self._async_fetch_one_extra_task(),
                name=f"aidot_extras_{id(self)}")
        except Exception:
    ...
    async def _async_fetch_one_extra(self, key: str | None = None) -> None:
        ...
        try:
            ...
            _LOGGER.debug("camera extras (%s) timed out for %s",
                          key, self.device_client.device_id)
            return
        except Exception as exc:
            _LOGGER.debug("camera extras (%s) failed for %s: %s",
                          key, self.device_client.device_id, exc)
            return
        if value is None:
        ...
        if first:
            _LOGGER.info("camera %s reported %s details",
                         self.device_client.device_id, key)
    ...
    def _refresh_failed(self, reason: str) -> DeviceStatusData:
        ...
        _LOGGER.debug(
            ...
            self.device_client.device_id, self._consecutive_failures,
            CONSECUTIVE_FAILURES_BEFORE_UNAVAILABLE, reason,
        )
        return self.device_client.status
