...
# DeviceState is upstream's own connection-state enum, re-exported by the camera
# layer.  Import it from there: `aidot` is an implementation detail of
# aidot_cameras and an undeclared dependency for this integration.
...
def _is_connected(device_client: Any) -> bool:
    ...
    Both flavours of client track this the same way, in the private ``_state``
    the camera client inherits, so there is one branch and not two.  Reading a
    private attribute is deliberate: the library exposes no public equivalent.
    If a future version renames it this returns False, which costs a cheap no-op
    reconnect attempt every poll (upstream's ``async_login`` returns immediately
    unless the state is IDLE) and nothing else - ``has_lan_state`` does not
    depend on it, precisely so that a rename cannot pin every light to unknown.
    """
