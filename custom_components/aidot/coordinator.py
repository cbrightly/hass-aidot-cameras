...
class AidotCameraUpdateCoordinator(AidotDeviceUpdateCoordinator):
    ...
    def _schedule_camera_extras_inner(self) -> None:
        ...
        if getattr(self, "_extras_task", None) is not None:
        ...
        # One action is fetched per window, so a 30-minute window means the
        # full set takes an hour and a half to appear on a fresh start. Fill
        # quickly until every key has a value, then settle to the slow cadence:
        # these change when somebody edits a setting, not on their own.
        have = len(getattr(self, "camera_extras", None) or {})
        due_in = (EXTRAS_FILL_SECONDS if have < len(self._EXTRAS_KEYS)
                  else EXTRAS_REFRESH_SECONDS)
        self._extras_due = now + due_in
        _LOGGER.debug("extras[%s]: scheduling a fetch (next in %ds)", dev, due_in)
        self._extras_task = hass.async_create_background_task(
            self._async_fetch_one_extra(), name=f"aidot_extras_{id(self)}")

    async def _async_fetch_one_extra(self) -> None:
        """Fetch the next extra in the rotation, then let entities know."""
        try:
    ...
    async def async_refresh_camera_extras(self, *, force: bool = False) -> None:
        """Fetch every extra now and wait for it. Used after a write."""
        if getattr(self.device_client, "is_battery_camera", False):
            return
        if not isinstance(getattr(self, "camera_extras", None), dict):
            self.camera_extras = {}
        for _ in self._EXTRAS_KEYS:
