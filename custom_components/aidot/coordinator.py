...
# DeviceState is upstream's own connection-state enum; the camera layer does not
# re-export it, and upstream is a guaranteed dependency of aidot_cameras.
from aidot.device_client import DeviceState
...
def _is_connected(device_client: Any) -> bool:
    """Return True when the device's control connection is up and authenticated.

    The camera client exposes ``connect_and_login``; upstream's client tracks a
    ``DeviceState`` instead, where AUTHENTICATED is the equivalent.
    """
    flag = getattr(device_client, "connect_and_login", None)
    if flag is not None:
        return bool(flag)
...
class AidotDeviceUpdateCoordinator(DataUpdateCoordinator[DeviceStatusData]):
    ...
    def has_lan_state(self) -> bool:
        """True when real device state has been received over the LAN."""
        return self._has_lan_state or _is_connected(self.device_client)
    ...
    def _handle_status_update(self, status: DeviceStatusData) -> None:
        # Only the device's own LAN push reaches here, so this is the signal
        # that the status object now holds real values rather than defaults.
        self._has_lan_state = True
