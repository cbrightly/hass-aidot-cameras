...
#: Sound detection, wifi and SD details change when someone edits a
#: setting, so they are polled far less often than device attributes.
EXTRAS_REFRESH_SECONDS = 30 * 60
#: While the set is still incomplete, fill it at the poll's own pace
#: rather than making a user wait an hour and a half for a sensor.
EXTRAS_FILL_SECONDS = 30
...
def supports_ptz(coordinator) -> bool:
    """Whether this camera can actually pan or tilt.

    Prefer the advertised PTZ direction codes -- the authoritative capability
    -- and fall back to the model id, because ptz_directions is sometimes empty
    at setup and arrives on a later refresh; gating on it alone would strip
    controls from the one camera that needs them during that window.

    Shared by button.py (which PTZ buttons to show) and switch.py (whether the
    auto-tracking switch ships at all). One definition: a control this firmware
    cannot perform is acknowledged and ignored, so a stale second copy of this
    rule produces a switch that silently does nothing.
    """
...
#: How many refreshes in a row must fail before the entities go unavailable.
#: One failed request is normal against this cloud; a run of them is not, and a
#: user staring at a battery reading from six hours ago deserves to be told.
CONSECUTIVE_FAILURES_BEFORE_UNAVAILABLE = 3
...
class AidotCameraUpdateCoordinator(AidotDeviceUpdateCoordinator):
    ...
    def __init__(
        ...
        # Consecutive failed cloud refreshes. See _refresh_failed: the
        # entities only go unavailable after a run of them, not one blip.
        ...
        # Camera "extras": sound detection, wifi and SD-card details. These are
        # not in the cloud device payload -- each is an MQTT round trip to the
        # camera -- so they are fetched on a much slower cadence than the
        # 5-minute attribute poll, and never from a battery camera, which is
        # asleep and would answer nothing while we hold the request open.
        self.camera_extras: dict[str, Any] = {}
        self._extras_due: float = 0.0
        self._extras_index: int = 0
        self._extras_task = None
        self._extras_inflight: bool = False
        self._extras_tried: set[str] = set()
        # One line per camera at setup, so "is the new code actually loaded?"
        # is answerable from the log instead of inferred from silence.
        _LOGGER.debug(
            "extras: rotation armed for %s (%s)",
            getattr(device_client, "device_id", "?"),
            "battery"
            if getattr(device_client, "is_battery_camera", False)
            else "mains",
        )
    ...
    async def _async_list_when_the_session_lands(
        ...
        while time.monotonic() < deadline:
            ...
            if result is not None:
                return result
        _LOGGER.debug(
            "No session to list %s arrived within %ss",
            getattr(dc, "device_id", "?"),
            SD_SESSION_WAIT_S,
        )
        return None
        ...
        async with self._sd_lock:
            ...
            if open_session and self._sd_opener is not None:
                try:
                    ...
                    _LOGGER.debug(
                        "Could not open a session to list %s: %s", dc.device_id, exc
                    )
            try:
                result = await fn(days=SD_LOOKBACK_DAYS)
                ...
                _LOGGER.debug("On-device listing failed for %s: %s", dc.device_id, exc)
                return False
            ...
            # Consecutive silences, so a camera that never answers stops being
            # re-asked on every session while a one-off keeps its fast retry.
            # Reset by any answer, including one that reports an empty card.
            streak = (
                0
                if result.answered
                else ((self.sd_cache.unanswered_streak if self.sd_cache else 0) + 1)
            )
    ...
    async def _async_update_data(self) -> DeviceStatusData:
        ...
        #
        # A refresh that keeps failing has to surface. This used to swallow
        # everything and hand back the last known status, which meant
        # last_update_success could never go False: through a cloud outage every
        # sensor and the occupancy binary_sensor kept serving frozen values and
        # stayed green, with nothing anywhere saying otherwise.
        #
        # It is deliberately not a hair trigger. These cameras sit behind a
        # cloud that blips, and dropping the whole fleet on one failed request
        # would be worse than a slightly stale battery reading, so it takes
        # CONSECUTIVE_FAILURES_BEFORE_UNAVAILABLE in a row and any success
        # resets the count.
        try:
            ...
            return self._refresh_failed(f"{exc}")

        # A falsy device is a failure too. It used to fall through to the same
        # "return the old status" line and freeze just as silently as a raise.
        if not device:
            return self._refresh_failed("the cloud returned no device record")

        self.device_client.update_status_from_device(device)
        self._consecutive_failures = 0
        self._schedule_camera_extras()
        return self.device_client.status

    #: The extras are fetched one at a time, round-robin, so a poll never
    #: waits on more than one request.
    _EXTRAS_KEYS = ("sound", "wifi", "sd", "detect", "alarm")

    def _schedule_camera_extras(self) -> None:
        """Never let scheduling break the poll (see _schedule_camera_extras_inner)."""
        try:
            self._schedule_camera_extras_inner()
        except Exception:  # pragma: no cover - defensive
            _LOGGER.debug(
                "scheduling camera extras failed for %s",
                getattr(self.device_client, "device_id", "?"),
                exc_info=True,
            )

    def _schedule_camera_extras_inner(self) -> None:
        """Kick off an extras fetch WITHOUT blocking the poll.

        The first version awaited three MQTT round trips inline. Each has an
        8 s timeout, so a single poll could sit for 24 s per camera on the
        critical path -- and in practice it never completed at all: the fetch
        was entered on every mains camera and no result was ever recorded.

        So it runs as a background task now, one action per cycle, with a hard
        timeout. A fetch that hangs can no longer stall the coordinator, and a
        hang is visible as a timeout rather than as silence.
        """
        dev = getattr(self.device_client, "device_id", "?")
        if getattr(self.device_client, "is_battery_camera", False):
            _LOGGER.debug("extras[%s]: skipped, battery camera", dev)
            return  # asleep; do not wake it
        if getattr(self, "_extras_inflight", False):
            _LOGGER.debug("extras[%s]: skipped, one already in flight", dev)
            return
        now = time.monotonic()
        if now < getattr(self, "_extras_due", 0.0):
            _LOGGER.debug(
                "extras[%s]: skipped, not due for %.0fs",
                dev,
                getattr(self, "_extras_due", 0.0) - now,
            )
            return
        hass = getattr(self, "hass", None)
        if hass is None:
            _LOGGER.debug("extras[%s]: skipped, no hass", dev)
            return
        # Only claim the slot once we are certain we can act on it. Setting the
        # due time before the hass check would silently skip the next 30 min on
        # a coordinator that could never schedule in the first place.
        # One action is fetched per window, so on the slow cadence alone a fresh
        # start would take two and a half hours to show a full set. Until every
        # key has been asked for, make the next poll due immediately instead.
        #
        # EXTRAS_FILL_SECONDS is "due at the next poll", not a delay anyone
        # waits: the only caller is _async_update_data on
        # UPDATE_CAMERA_ATTRS_INTERVAL (5 minutes), so any value below that is
        # indistinguishable. The real fill time is one key per poll -- five keys
        # is about 25 minutes -- not 150 seconds. Anything that adds a key adds
        # five minutes to that, so keep this number and _EXTRAS_KEYS in step.
        # A key counts as covered once it has been ASKED FOR, not once it has a
        # value: a camera that answers None to one action stores nothing, and
        # counting stored values left it at the fill rate forever - six times the
        # intended request rate, permanently, on the models least able to serve it.
        covered = (getattr(self, "_extras_tried", None) or set()) | set(
            getattr(self, "camera_extras", None) or {}
        )
        due_in = (
            EXTRAS_FILL_SECONDS
            if len(covered) < len(self._EXTRAS_KEYS)
            else EXTRAS_REFRESH_SECONDS
        )
        self._extras_due = now + due_in
        _LOGGER.debug("extras[%s]: scheduling a fetch (next in %ds)", dev, due_in)
        # Claim the slot BEFORE creating the task, and never write the guard
        # afterwards. `async_create_background_task` eager-starts the coroutine,
        # so a fetch that returns without suspending -- an older wheel with no
        # such getter, no live session -- runs its whole body, and its `finally`,
        # inside the call below. Guarding on the task handle instead meant that
        # `finally` cleared the slot first and the finished Task was assigned
        # back over it, leaving a permanent "one already in flight" that no
        # later poll could clear.
        self._extras_inflight = True
        try:
            task = hass.async_create_background_task(
                self._async_fetch_one_extra_task(), name=f"aidot_extras_{id(self)}"
            )
        except Exception:
            self._extras_inflight = False
            raise
        # Only a still-running task is worth a handle; a finished one is not.
        self._extras_task = None if task.done() else task

    def cancel_camera_extras(self) -> None:
        """Stop an in-flight extras fetch. Called on config-entry unload.

        ``hass.async_create_background_task`` is only auto-cancelled at Home
        Assistant shutdown, so on a reload -- options change, reauth, a HACS
        update -- a fetch inside its 25 s wait kept running against a torn-down
        coordinator, held it and its device client alive past unload, and then
        wrote to ``camera_extras`` and notified listeners on a dead object.
        """
        task = getattr(self, "_extras_task", None)
        if task is not None and not task.done():
            task.cancel()
        self._extras_task = None
        self._extras_inflight = False

    async def _async_fetch_one_extra_task(self) -> None:
        """Scheduler entry point: owns the in-flight slot.

        The slot is released here and not inside the fetch itself, because
        ``async_refresh_camera_extras`` awaits the fetch inline -- releasing it
        there would hand the next poll a free slot while a background fetch was
        still running."""
        try:
            await self._async_fetch_one_extra()
        finally:
            self._extras_inflight = False
            self._extras_task = None

    async def _async_fetch_one_extra(self, key: str | None = None) -> None:
        """Fetch one extra, then let entities know.

        With no key, takes the next in the rotation and advances it. With a key,
        fetches just that one and leaves the rotation alone, so a write can read
        back what it changed without disturbing the background cadence.
        """
        if not isinstance(getattr(self, "camera_extras", None), dict):
            self.camera_extras = {}
        if key is None:
            idx = getattr(self, "_extras_index", 0) % len(self._EXTRAS_KEYS)
            self._extras_index = idx + 1
            key = self._EXTRAS_KEYS[idx]
        # Record the attempt, not just the result. A camera that answers None to
        # one action would otherwise never complete the set, so the cadence would
        # stay at the fill rate forever on exactly the models that cannot serve it.
        if not isinstance(getattr(self, "_extras_tried", None), set):
            self._extras_tried = set()
        self._extras_tried.add(key)
        name = {
            "sound": "async_get_sound_detection",
            "wifi": "async_get_wifi_info",
            "sd": "async_get_sd_card_info",
            "detect": "async_get_detection_types",
            "alarm": "async_get_auto_alarm",
        }[key]
        # Resolved by name: these landed in the library after the version
        # this integration requires, so an older wheel simply has no such
        # method and must not raise on every poll.
        getter = getattr(self.device_client, name, None)
        if getter is None:
            return
        try:
            value = await asyncio.wait_for(getter(), timeout=25)
        except TimeoutError:
            _LOGGER.debug(
                "camera extras (%s) timed out for %s", key, self.device_client.device_id
            )
            return
        except Exception as exc:
            _LOGGER.debug(
                "camera extras (%s) failed for %s: %s",
                key,
                self.device_client.device_id,
                exc,
            )
            return
        if value is None:
            # Unknown, not empty. Leave any previous value in place -- one
            # unanswered request must not blank an entity that was reading.
            return
        first = key not in self.camera_extras
        self.camera_extras[key] = value
        if first:
            _LOGGER.info(
                "camera %s reported %s details", self.device_client.device_id, key
            )
        # Entities for these are created lazily when the value first
        # appears, so the platforms have to be told. Guarded because this
        # also runs against a coordinator built without the base class's
        # listener machinery.
        notify = getattr(self, "async_update_listeners", None)
        if notify is not None:
            # Deliberately NOT wrapped in try/except AttributeError: that also
            # caught anything the listener callbacks raised, so a platform whose
            # entity-creation hook hit an unexpected shape failed silently and
            # the entities were simply never created.
            notify()

    async def async_refresh_camera_extras(self, *, only: str | None = None) -> None:
        """Fetch extras now and wait for them. Used to read back after a write.

        ``only`` narrows it to a single key. A write changes one thing, and this
        runs inside the service call on a platform declaring
        ``PARALLEL_UPDATES = 1``: re-reading all three at 25 s each could block
        every other switch on the account for over a minute to confirm one flag.
        """
        if getattr(self.device_client, "is_battery_camera", False):
            return
        if not isinstance(getattr(self, "camera_extras", None), dict):
            self.camera_extras = {}
        if only is not None:
            await self._async_fetch_one_extra(only)
            return
        for key in self._EXTRAS_KEYS:
            await self._async_fetch_one_extra(key)

    def _refresh_failed(self, reason: str) -> DeviceStatusData:
        """Count a failed refresh, and give up once there have been enough."""
        self._consecutive_failures += 1
        if self._consecutive_failures >= CONSECUTIVE_FAILURES_BEFORE_UNAVAILABLE:
            raise UpdateFailed(
                f"camera {self.device_client.device_id} status refresh failed "
                f"{self._consecutive_failures} times in a row: {reason}"
            )
        _LOGGER.debug(
            "Camera status refresh failed for %s (%d/%d, will retry): %s",
            self.device_client.device_id,
...
class AidotDeviceManagerCoordinator(DataUpdateCoordinator[None]):
    ...
    async def _async_update_data(self) -> None:
        ...
        current_lights = {d[CONF_ID]: d for d in all_devices if _is_light_device(d)}
        ...
        current_cameras = {d[CONF_ID]: d for d in all_devices if _is_camera_device(d)}
    ...
    def _refresh_camera_attributes(
        ...
        for dev_id, device in current_cameras.items():
            ...
            streaming = getattr(dc, "stream_rtsp_url", None) is not None or bool(
                getattr(dc, "_streaming_active", False)
            )
    ...
    def _sync_camera_coordinators(self, current: dict[str, dict[str, Any]]) -> None:
        self._sync_coordinators(
            cast(dict[str, AidotDeviceUpdateCoordinator], self.camera_coordinators),
            current,
            is_camera=True,
        )
    ...
    async def _async_attach_local_control(
        ...
        async with self._lan_lock:
            ...
            for dev_id in pending:
                ...
                try:
                    lan = CameraLanClient(device, self.client.login_info, ip=ip)
    ...
    async def _async_init_coordinator(
        ...
        try:
            ...
            _LOGGER.warning(
                ...
                _device_id(coord.device_client),
                exc,
            )
...
def get_camera_coordinators(
    hass: HomeAssistant,
) -> dict[str, "AidotCameraUpdateCoordinator"]:
