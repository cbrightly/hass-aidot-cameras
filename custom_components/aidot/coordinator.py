...
class AidotCameraUpdateCoordinator(AidotDeviceUpdateCoordinator):
    ...
    async def _async_setup(self) -> None:
        ...
        # Reuse one persistent MQTT connection for camera commands AND stream
        # signaling instead of reconnecting per op (matches the app). On by
        # default. Set the opt EXPLICITLY (True or False) so a user who turns the
        # toggle OFF overrides the library's default-on; account-level, so setting
        # it on this device_client applies to the shared connection.
        self.device_client._persistent_mqtt_opt = bool(self.config_entry.options.get(
