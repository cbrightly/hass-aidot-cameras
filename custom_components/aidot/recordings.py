...
def day_windows(now_ms: int, days: int, tz: tzinfo) -> list[tuple[str, int, int]]:
...
async def day_summaries(
    ...
    return [
        (label, s, e, n)
        # strict: asyncio.gather returns one result per input, in order, so
        # these lengths are equal by construction. If that ever stops being
        # true, a silently truncated day list is the worst way to find out.
        for (label, s, e), n in zip(windows, counts, strict=True)
        if n is None or n > 0
    ]
