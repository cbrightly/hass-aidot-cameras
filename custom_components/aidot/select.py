...
class AidotSelectDescription(SelectEntityDescription):
    ...
    #: camera gets it. A camera that cannot work the setting must get no entity
    #: at all rather than one stuck at unknown or, worse, one that accepts a
    #: value the camera ignores.
...
def _declares(client, name: str) -> bool:
    """Does this MODEL declare the property in its cloud profile?

    True when the profile is unreadable, because an empty set means unknown
    rather than "declares nothing" -- a camera whose profile failed to parse
    should not quietly lose a control.
    """
    declared = getattr(getattr(client, "info", None), "declared_properties", None)
    return not declared or name in declared


def remove_unsupported_linger_select(hass, dev_id: str, client) -> None:
    """Delete a duration select 2.23.0 created on a model that cannot work it.

    Dropping the entity is not enough: Home Assistant keeps the registry row and
    shows it as a permanently unavailable control that still appears in the UI
    and in automations. Removal is keyed on the model NOT DECLARING the
    property, never on the absence of a reported value - a camera that simply
    has not answered yet must keep its entity rather than have it deleted and
    recreated on every restart.
    """
    if _declares(client, "LingerDuration"):
        return
    reg = er.async_get(hass)
    eid = reg.async_get_entity_id("select", DOMAIN, f"{dev_id}_light_linger_duration")
    if eid:
        _LOGGER.debug("removing unsupported duration select %s", eid)
        reg.async_remove(eid)


#: How the floodlight behaves when it triggers, by the names the library uses.
#: The model profile declares exactly two: Constant=0 and Flash=1.
LIGHT_BEHAVIOR_OPTIONS = ("constant", "flash")


def _behavior_option(status) -> str | None:
    """The camera's behaviour as one of our options, or None.

    The library carries a value it does not model through as itself, so a
    firmware that grows a third mode arrives here as a bare number. Showing it
    would be an option this select cannot round-trip.
    """
    v = getattr(status, "light_behavior", None)
    return v if v in LIGHT_BEHAVIOR_OPTIONS else None
...
CAMERA_SELECTS: tuple[AidotSelectDescription, ...] = (
    AidotSelectDescription(
        ...
        # Gated on the MODEL DECLARING it, not on the cloud reporting a value.
        # 2.23.0 gated on the reported value and offered this on the A000088,
        # whose cloud properties carry LingerDuration = "30" while the camera,
        # asked directly over LAN, has no such attribute and ignores writes to
        # it. The vendor app gates on the profile for exactly this reason.
        is_supported=lambda s, c: (
            getattr(s, "light_linger_duration", None) is not None
            and _declares(c, "LingerDuration")),
    ),
    # How the light behaves when it triggers. Held back until 2026-09-08:
    # the write acked and the value never moved, which read as a problem with
    # the attribute. It was a battery camera never getting time to wake before
    # the command arrived behind its own wake (fixed in library 1.0.0rc18), and
    # it now lands on a sleeping camera - verified by read-back, which is what
    # the resolution-select rule asks for.
    AidotSelectDescription(
        key="light_behavior",
        translation_key="light_behavior",
        icon="mdi:flash",
        entity_category=EntityCategory.CONFIG,
        options=list(LIGHT_BEHAVIOR_OPTIONS),
        get_current_option=_behavior_option,
        async_select_option_fn=lambda c, v: c.async_set_light_behavior(v),
        is_supported=lambda s, c: (
            getattr(s, "light_behavior", None) is not None
            and _declares(c, "lightBehavior")),
    ),
    AidotSelectDescription(
        key="night_vision",
...
async def async_setup_entry(
    ...
    def _add_new_selects() -> None:
        ...
        for dev_id, c in coordinator.camera_coordinators.items():
            remove_unsupported_linger_select(
                hass, dev_id, getattr(c, "device_client", None))
        for dev_id, c in coordinator.camera_coordinators.items():
