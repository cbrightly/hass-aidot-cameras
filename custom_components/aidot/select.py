...
class AidotResolutionSelect(RestoreEntity, AidotCameraSelect):
    ...
    default on a fresh install so it never shows "unknown").

    The setting is delivered over the live streaming session, so it can only
    arrive while the camera is streaming. As of python-aidot-cameras 0.15.2 a
    choice made while the camera is idle is remembered by the library and sent
    when a session next starts, which is what makes showing the new value here
    honest. Before that it was discarded outright and nothing re-sent it, so
    this entity displayed a setting the camera had never received - the usual
    case, since a camera is idle far more often than it is watched.
    """
