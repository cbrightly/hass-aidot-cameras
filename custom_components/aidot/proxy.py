...
* **Auth**: the view is unauthenticated (``requires_auth = False``) because the
  ...
  source signs each playback URL with an HMAC over ``device + event + expiry``
  (:func:`sign_playback_url`) and the view verifies it (:func:`_verify_sig`), so
  a leaked or guessed ``eventUuid`` can't be replayed against this endpoint past
  the expiry window.  The key is persisted in HA storage
  (:func:`async_load_url_secret`), so a signed link survives a restart.
"""
...
# A motion push fires the instant motion is detected, but the recorded clip
# takes a few seconds to finish uploading to the cloud, so a tap on the push can
# reach this view before the clip's playback URL exists. Poll for it a bounded
# number of times (total ~ retries * delay) before giving up, so the common
# "tapped the notification" case just plays. Override with
# AIDOT_CLIP_READY_RETRIES / AIDOT_CLIP_READY_DELAY_S.
_CLIP_READY_RETRIES = int(os.environ.get("AIDOT_CLIP_READY_RETRIES", "5"))
_CLIP_READY_DELAY_S = float(os.environ.get("AIDOT_CLIP_READY_DELAY_S", "2.0"))
# Retry-After hint (seconds) on the "clip not ready yet" response.
_CLIP_RETRY_AFTER_S = 5
...
# secret and the view verifies it. This module-level value is a per-process
# fallback ONLY - a random secret good until async_load_url_secret runs at entry
# setup and swaps in the persisted one (or creates and persists it, on first
# run). With that in place, a signed URL - including the week-long links inside
# push notifications - survives an HA restart up to its own TTL rather than
# dying at the next process start.
...
_URL_TTL = 6 * 3600
# Longer window for a link put INSIDE a push notification, which is tapped
# whenever the user next picks up their phone rather than moments after
# browsing. The secret is persisted (see async_load_url_secret below), so a
# minted link stays valid for this full window regardless of an HA restart.
NOTIFY_URL_TTL = 7 * 24 * 3600

STORAGE_KEY = "aidot_url_secret"
STORAGE_VERSION = 1

# Serializes the load-or-create body below so two config entries setting up
# concurrently can't both see an empty store, mint DIFFERENT secrets, and
# write the same file through independent Store instances (each Store has its
# own write lock, and atomic_writes defaults off, so interleaved writes could
# corrupt the JSON). _SECRET_LOADED short-circuits every call after the first
# in this process: the store only needs to be consulted once per process, not
# once per config-entry reload, since _URL_SECRET is a single module global.
_SECRET_LOCK = asyncio.Lock()
_SECRET_LOADED = False


async def async_load_url_secret(hass) -> None:
    """Load (or create and persist) the URL-signing secret.

    The secret was per-process, which invalidated every signed clip link -
    including the week-long links inside push notifications - at each restart.
    Persisted under .storage it lives in the same trust domain as Home
    Assistant's own auth store; replay of a leaked URL is bounded by the
    link's TTL instead of the next restart. Idempotent and cheap to call
    again: once a secret has been loaded or created in this process, later
    calls (e.g. from a second config entry, or a reload) return immediately
    without touching the store. Tolerant of a missing, corrupt, or
    unwritable store (regenerates and keeps going on the in-memory secret
    rather than failing setup). A failed persist is not retried until the
    next restart - the freshly generated secret still serves this process,
    just without surviving it.
    """
    global _URL_SECRET, _SECRET_LOADED
    ...
    async with _SECRET_LOCK:
        ...
        except Exception:  # corrupt store: regenerate below rather than fail setup
            _LOGGER.warning("URL-secret store unreadable; issuing a fresh secret")
        secret = None
        ...
        if secret is None:
            ...
            try:
                await store.async_save({"secret": secret.hex()})
            except Exception:
                _LOGGER.warning(
                    "could not persist the clip-link signing key; "
                    "links will not survive a restart"
                )
        _URL_SECRET = secret
        _SECRET_LOADED = True


# A clip is meant to be PLAYED where it is opened, not saved. aiohttp's
# FileResponse infers video/mp4 from the extension but sends no
# Content-Disposition, and a bare MP4 navigation is exactly the case a WebView
# is entitled to treat as a download - which is how the notification's
# clickAction can end up saving a file instead of playing it. Saying "inline"
# explicitly is the standards-correct answer for content intended for playback
# and costs nothing on clients that already got it right.
#
# The filename is a fixed literal rather than the event id: the id comes from
# the cloud, and nothing user-influenced belongs in a response header.
_CLIP_HEADERS = {"Content-Disposition": 'inline; filename="clip.mp4"'}


def _clip_response(cache_path: str) -> "web.FileResponse":
    """Serve a cached clip for inline playback."""
    return web.FileResponse(cache_path, headers=_CLIP_HEADERS)


def _clip_error_response(status: str) -> web.Response:
    """Map a non-ok transcode outcome to its HTTP response.

    ``"not_ready"`` means the clip's playback URL could not be resolved yet -
    the cloud has not finished the recording - so no transcode was even
    attempted. That is transient and worth retrying, and it is a different thing
    from ffmpeg failing. Returning the same 502 "clip transcode failed" for both
    (the old behaviour) told a user tapping a motion push seconds after the
    event that transcoding failed when nothing was transcoded. Answer 503 with
    Retry-After so the client - and a human reading it - knows to try again
    shortly.

    Anything else is a genuine encode failure: ffmpeg ran and every plan failed.
    """
    if status == "not_ready":
        return web.Response(
            status=503,
            headers={"Retry-After": str(_CLIP_RETRY_AFTER_S)},
            text="clip not ready yet, try again shortly",
        )
...
def sign_playback_url(
    ...
    *,
    ...
    """
    exp = int(
        (time.time() if now is None else now) + (_URL_TTL if ttl is None else ttl)
    )
...
async def async_resolve_event_url(
    coord: AidotCameraUpdateCoordinator, event_uuid: str
) -> str | None:
...
class AidotVideoProxyView(HomeAssistantView):
        ...
        try:
            async with lock:
                ...
                async with _TRANSCODE_SEM:
                    await self._transcode_to_cache(
                        device_id, event, cache_path, hevc=True, wait_ready=False
                    )
        except Exception as exc:
            _LOGGER.debug("aidot clip prewarm failed for %s: %s", event, exc)
        finally:
            # Evict only when nobody holds OR awaits it. release() wakes a queued
            # waiter before lock.locked() flips back, so a bare not-locked check
            # can drop a lock a pending waiter still references - letting a second
            # ffmpeg encode race the same cache path. _waiters stays non-empty
            # until the woken waiter actually resumes, so it closes that window.
    ...
    async def get(self, request: web.Request) -> web.StreamResponse:
        ...
        if await self.hass.async_add_executor_job(_touch_if_usable, cache_path):
            return _clip_response(cache_path)
        ...
        try:
            async with lock:
                ...
                async with _TRANSCODE_SEM:
                    status = await self._transcode_to_cache(
                        device_id, event, cache_path, hevc=webkit
                    )
                if status != "ok":
                    return _clip_error_response(status)
                return _clip_response(cache_path)
        finally:
    ...
    async def _transcode_to_cache(
        ...
        *,
        ...
        """Produce a clip MP4 in the cache; return the outcome.

        Returns ``"ok"`` when a usable file was cached, ``"not_ready"`` when the
        clip's playback URL could not be resolved (the cloud has not finished the
        recording yet - no ffmpeg was run), or ``"failed"`` when a URL resolved
        but every encoder plan failed. ``get`` maps these to 200 / 503 / 502 so a
        clip that is merely not ready yet is not reported as a transcode failure.

        ``wait_ready`` polls briefly for the URL (the interactive path); prewarm
        passes it False so warming a not-yet-uploaded clip does not block.
        ...
        try:
            url = (
                await self._resolve_when_ready(device_id, event)
                if wait_ready
                else await self._resolve_url(device_id, event)
            )
            if not url:
                return "not_ready"
            urls = [url]
            ...
            while True:
                ...
                for plan_name, input_args, video_args in plans:
                    for candidate in urls:
                        ...
                        # Redact the signed, time-limited media URL from the
                        # argv log (bearer credential) - same policy as the
                        # event/thumbnail signed-URL truncation.
                        _safe_argv = [
                            "<signed-url>" if a == candidate else a for a in argv
                        ]
                        _LOGGER.debug(
                            "aidot clip plan=%s: %s", plan_name, " ".join(_safe_argv)
                        )
                ...
                if not fresh:
                    # We already resolved a URL and tried to encode it; a URL we
                    # can no longer refresh is an encode failure, not "not ready".
                    return "failed"
                urls = [fresh]
        finally:
            await self.hass.async_add_executor_job(_unlink, tmp_path)
    ...
    async def _resolve_when_ready(self, device_id: str, event: str) -> str | None:
        """Resolve the clip URL, retrying briefly while the cloud finishes it.

        A motion push fires the moment motion is detected; the recorded clip
        takes a few seconds to upload, so a tap right after the push finds no
        playback URL yet. Poll a bounded number of times before giving up, so
        the common case just plays instead of showing an error.
        """
        for attempt in range(_CLIP_READY_RETRIES + 1):
            url = await self._resolve_url(device_id, event)
            if url:
                return url
            if attempt < _CLIP_READY_RETRIES:
                await asyncio.sleep(_CLIP_READY_DELAY_S)
        return None
