...
* **Auth**: the view is unauthenticated (``requires_auth = False``) because the
  media-browser ``<video>`` request carries no HA ``authSig``.  Access is gated
  by the unguessable ``eventUuid`` plus a fresh signed CloudFront URL - the same
  posture as the HLS proxy this replaces.
"""
...
class AidotVideoProxyView(HomeAssistantView):
    ...
    # the unguessable eventUuid; the only thing fed to ffmpeg is a CloudFront URL
    # we resolve server-side (never a client-supplied one) - so no SSRF surface.
    requires_auth = False
