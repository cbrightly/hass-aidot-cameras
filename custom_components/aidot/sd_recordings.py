...
def map_days(
    hours: Optional[bytes], start_ts: float, tz: tzinfo
    ...
    return [(label, sorted(by_day[label])) for label in sorted(by_day, reverse=True)]


def sd_card_known_absent(
    cache: Optional[SdCache], card_present: Optional[bool]
) -> bool:
...
def sd_empty_message(
    cache: Optional[SdCache], card_present: Optional[bool] = None
) -> str:
    ...
    if cache is None:
        return (
            "Not listed yet - press the 'Refresh on-device recordings' "
    ...
    if not cache.answered:
        ...
        # may not exist.
        return (
            "The camera did not answer when asked what it holds. It may "
            "have been offline or busy - try Refresh again."
        )
    if not cache.complete:
        return (
            "The camera sent a partial list - there may be more on the "
            "card than is shown."
        )
