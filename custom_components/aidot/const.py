...
# instant.  OFF by default: it opens a session on every mains camera at every
# restart and reload with no viewer present, which is indistinguishable from
# "why are all my cameras streaming?" - and each session holds a concurrency slot
# and keeps the camera decrypting.  Opt in if instant first-view matters more.
CONF_STARTUP_PREWARM = "startup_prewarm"
DEFAULT_STARTUP_PREWARM = False
